...
midiInput.addEventListener(
  ...
  async()=>{
    ...
    try{
      ...
      /* 합주곡인지 솔로곡인지 묻는다.
         솔로(악기 하나)면 파트를 전부 합쳐 세 트랙으로 고르게 나눈다 —
         아르미스는 한 사람이 트랙 셋까지라, 한 사람이 다 연주하려면
         트랙마다 3000자 안에 들어와야 한다. */
      const solo =
        await showConfirm({
          eyebrow:"midi",
          title: t("midi.modeTitle"),
          text: t("midi.modeText"),
          okText: t("midi.modeSolo"),
          cancelText: t("midi.modeEnsemble")
        });
      ...
      const song =
        await convertMidi(buffer, solo ? "solo" : "ensemble");
