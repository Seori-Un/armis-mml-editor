...
const barStartButton =
  document.getElementById("barStart");

const barEndButton =
  document.getElementById("barEnd");
...
function moveOutputResize(event){
  ...
  /* 0까지 내려가게 둔다 — 바닥까지 끌면 놓는 순간
     출력 창이 통째로 접힌다(endOutputResize 참고). */
  const minHeight = 0;
...
/* 접기 전의 높이. 다시 펼칠 때 이 높이로 돌아온다. */
let lastOpenOutputHeight = null;
...
function endOutputResize(event){
  ...
  /* 바닥 근처까지 끌어내리고 놓았으면 통째로 감춘다.
     화면 아래 가운데의 ▲ 손잡이로 다시 꺼낼 수 있다. */
  const height =
    output.getBoundingClientRect().height;

  if(height < 50){

    lastOpenOutputHeight =
      startHeight >= 120
        ? startHeight
        : lastOpenOutputHeight;

    /* 다음에 펼칠 때 찌그러진 높이가 남지 않게 지운다 */
    output.style.maxHeight =
      lastOpenOutputHeight
        ? lastOpenOutputHeight + "px"
        : "";
...
/* ── 트랙 목록 접기/펼치기 · 출력 창 다시 열기 ──
   상태는 body의 클래스 하나로 관리한다. 그리기는 전부 CSS가 한다. */

const railCollapseButton =
  document.getElementById("railCollapse");

const railExpandButton =
  document.getElementById("railExpand");

const outputExpandButton =
  document.getElementById("outputExpand");

if(railCollapseButton){

  railCollapseButton.addEventListener(
    "click",
    ()=>document.body.classList.add(
      "rail-collapsed"
    )
...
if(railExpandButton){
  ...
  railExpandButton.addEventListener(
    "click",
...
if(outputExpandButton){
  ...
  outputExpandButton.addEventListener(
    "click",
    ()=>document.body.classList.remove(
      "output-collapsed"
    )
...
/* ── 폰에서의 트랙 목록 ──
   폰(≤720px)에서는 목록이 판 위에 겹쳐 뜨는 서랍이다(styles.css).
   ① 처음 열 때는 접어 둔다 — 좁은 화면의 주인공은 판이다.
   ② 서랍이 열린 채 판을 건드리면 도로 접는다 — 겹쳐 있어서
      안 접으면 판 왼쪽이 계속 가려진 채로 남는다. */

/* styles.css의 폰 조건과 반드시 같아야 한다 —
   폭이 모자란 세로 폰, 높이가 모자란 가로 폰 둘 다.
   matchMedia가 없는 곳(오래된 웹뷰, 시험 하네스)에서는
   폰 취급을 하지 않으면 그만이다 */
/* 폰은 뷰포트를 1280으로 못박아 통째로 축소하므로(index.html)
   여기 조건에 절대 걸리지 않는다. 이 판별식은 데스크톱에서
   마우스로 창을 좁게 줄인 경우만 잡는다 — 그래서 pointer:fine. */
const phoneQuery =
  typeof window.matchMedia === "function"
    ? window.matchMedia(
        "(pointer:fine) and (max-width:480px), (pointer:fine) and (max-height:500px)"
      )
    : { matches:false };
...
/* 가로 폰(높이 ≤500px)에서는 출력 창까지 접고 시작한다 —
   세로 395px에서 빈 MML 줄이 90px을 먹고 있을 이유가 없다.
   ▲ 손잡이로 언제든 꺼낼 수 있다. */
const shortQuery =
  typeof window.matchMedia === "function"
    ? window.matchMedia(
        "(pointer:fine) and (max-height:500px)"
      )
    : { matches:false };

if(shortQuery.matches){
...
/* ── 가볍게(저사양) 모드 ──
   느린 기기의 진짜 병목은 계산이 아니라 그리기다 — 노트마다
   그라디언트와 그림자를 칠하고, 창마다 유리 블러를 돌리는 값.
   body.lite 하나로 그 치장을 전부 걷는다(styles.css 맨 아래).

   기기가 스스로 판단해 조용히 켠다 — 메모리 4GB 이하 또는
   코어 4개 이하. deviceMemory는 크롬 계열에만 있고 사파리에는
   없어서, 둘 중 하나만 있어도 본다. 화면에 단추는 없다
   (예전에 있었지만 사전에 없는 글쇠가 그대로 찍혀 없앴다).
   localStorage 값은 그 시절 단추가 남긴 것 — 계속 존중한다. */

const LITE_STORE_KEY =
  "armis-mml-lite";
...
function detectLite(){
  ...
  try{
    ...
    const saved =
      localStorage.getItem(LITE_STORE_KEY);
...
function toggleTieFor(indices){
  ...
  /* 잇기 버튼은 사슬의 "머리"만 받는다 (화면에서 사슬은 한 음이므로).
     그런데 사슬 A를 다음 조각 B와 이으려면 tie를 켜야 할 음은 A의
     "꼬리"다 — 머리만 보던 예전 코드는 그래서 사슬끼리를 못 이었다.
     여기서는 고른 머리를 사슬 전체로 펼친 뒤, "앞 사슬의 꼬리 끝과
     뒷 사슬의 머리 시작이 맞닿은 이음매"를 찾아 잇는다. */

  /* 고른 머리 → 사슬(단위)로 펼치기. 같은 사슬이 두 번 들어오면 한 번만. */
  const seen = new Set();
  const units = [];

  for(const head of indices){
  ...
  units.sort(
    (a, b) =>
      track.notes[a[0]].bar - track.notes[b[0]].bar
  );

  const allNotes =
    units.flatMap(
      chain => chain.map(i => track.notes[i])
    );

  /* 그 자리에서 시작하는 노트 수 — 화음인지 판별 */
  const countAt = bar =>
    track.notes.filter(
      other =>
        Math.abs(other.bar - bar) < BAR_EPS
    ).length;

  /* ① 이음매 찾기: 앞 단위의 꼬리 끝 == 뒷 단위의 머리 시작 (같은 줄).
     화음 자리(그 시작에 다른 음이 또 있는 자리)는 잇지 않는다 —
     화음 진행 도중에 딴 음이 시작하는 모양은 아르미스 MML로 적을 수
     없기 때문이다. 화음은 아래 ②(화음 전체 늘이기)로만 다룬다. */
  const joins = [];

  for(let u = 0; u + 1 < units.length; u++){

    const tail =
      track.notes[units[u][units[u].length - 1]];

    const head =
      track.notes[units[u + 1][0]];
    ...
    if(
      tail.row === head.row &&
      Math.abs((tail.bar + tail.len) - head.bar) < BAR_EPS &&
      countAt(tail.bar) === 1 &&
      countAt(head.bar) === 1
    ){
  ...
  if(joins.length){
    ...
    selectedNote =
      track.notes.indexOf(allNotes[allNotes.length - 1]);
  ...
  /* ② 한 자리의 완전한 화음을 골랐고 아직 안 이어져 있다면 화음을
     늘인다 — 구성음마다 이어받을 조각을 새로 만들어 붙인다.
     (구성음 끝이 가지런해야 어디서 이어질지 정해진다) */
  ...
  const singleStart =
    notes.every(
      note =>
        Math.abs(note.bar - firstBar) < BAR_EPS
    );

  if(singleStart && !notes.some(note => note.tie)){

    const whole =
      track.notes.filter(
        other =>
          Math.abs(other.bar - firstBar) < BAR_EPS
      );

    if(whole.length !== notes.length){
    ...
    if(Math.max(...ends) !== Math.min(...ends)){
  ...
  /* ③ 더 이을 이음매가 없다. 이어진 조각이 있으면 이미 완성된 것을
     다시 누른 것 — 토글답게 전부 끊어 준다. */
  if(notes.some(note => note.tie)){
...
function renderInspectorTechniques(){
  ...
  /* ── 자동 탐지 단추 ──
     연주 데이터(길이·세기·짧은 앞음)를 보고 스타카토·악센트·
     꾸밈음을 찾아 붙인다. 결과는 칩으로 보이므로 사람이 하나씩
     확인하고 뗄 수 있다 — 변환 때 몰래 하는 게 아니다. */
  const autoBtn =
    document.createElement("button");
  ...
  inspectorTechniques.appendChild(autoBtn);
...
/* ============================================================
   기법 자동 탐지
   ============================================================
   변환 엔진에 잠들어 있는 판정 규칙(AUTO_TECHNIQUE=false로 꺼진
   것)과 같은 기준을 편집기 안에서, 사람이 시킬 때만 돌린다.
   결과가 노트 칩으로 보이고 되돌리기도 되므로 "짧은 음이 말없이
   사라지는" 문제가 없다.

   찾는 것 (그 악기에 있는 기법만):
     꾸밈음   아주 짧은 단음이 1~2반음 옆 본음 직전에 스치는 모양.
              짧은 음을 지우고 본음을 그 자리까지 늘린 뒤 *8/*9.
     스타카토 실제 길이가 자리(다음 음까지)의 55% 이하.
     악센트   세기가 주변 음들보다 뚜렷이 큰 음.

   비브라토·페르마타 등은 MIDI에 판단 근거가 없어 다루지 않는다.
   이미 *N 기법이 붙은 음은 건드리지 않는다(사람 선택 존중). ============================================================ */
function autoDetectTechniques(){

  const track =
    tracks[selectedTrack];

  if(!track || !track.notes.length){
    showToast(t("tech.autoNoNotes"));
    return;
  }

  const set =
    getTechniquesForInstrument(track.name);

  if(!set.staccato && !set.accent && !set.graceUp && !set.graceDown){
    showToast(t("tech.noneForInstrument"));
    return;
  }

  /* 다중 선택이 있으면 그 노트들만, 없으면 트랙 전체 */
  const onlyIdx =
    multiSelection.size
  ...
  const hasPost = n =>
    n.techs.some(
      tech => tech.code && tech.code.charAt(0) === "*"
    );
  ...
  const lenOf = n =>
    Math.max(1, Math.round(n.len * WHOLE));

  /* 시작 tick이 같은 노트들 = 화음 그룹 */
  const order =
    notes
      .map((n, i) => i)
      .sort((a, b) => startOf(notes[a]) - startOf(notes[b]));

  const groups = [];

  for(const i of order){

    const s = startOf(notes[i]);
    const last = groups[groups.length - 1];

    if(last && last.start === s){
      last.idx.push(i);
    }else{
      groups.push({ start: s, idx: [i] });
    }

  }

  const graceMaxTick = WHOLE / 32;

  /* 계획을 먼저 다 세우고, 바꿀 게 있을 때만 한 번에 적용한다 */
  const addTech = [];       // { index, role }
  const graceMoves = [];    // { mainIndex, graceIndex, role, newBar, addLen }
  ...
  for(let g = 0; g + 1 < groups.length; g++){
    ...
    if(cur.idx.length !== 1 || nxt.idx.length !== 1){
      continue;
    }

    const grace = notes[cur.idx[0]];
    const main = notes[nxt.idx[0]];

    if(removeIdx.has(cur.idx[0]) || removeIdx.has(nxt.idx[0])){
      continue;
    }

    if(onlyIdx && !onlyIdx.has(nxt.idx[0]) && !onlyIdx.has(cur.idx[0])){
      continue;
    }

    const graceSlot = nxt.start - cur.start;
    const after = groups[g + 2];

    const mainSlot =
      after
        ? after.start - nxt.start
        : Math.max(MIN_TICK, lenOf(main));

    const prevSlot =
      g > 0
        ? cur.start - groups[g - 1].start
        : Infinity;

    const graceLen = lenOf(grace);

    const diff =
      rowToKey(grace.row) - rowToKey(main.row);

    const role =
      diff > 0 ? "graceUp" : "graceDown";
    ...
    if(
      ...
      set[role] &&
      graceSlot >= MIN_TICK &&
      graceSlot <= graceMaxTick &&
      graceLen <= graceSlot + MIN_TICK &&
      graceSlot * 4 <= mainSlot &&
      graceSlot * 4 <= prevSlot &&
      !hasPost(main) &&
      main.techs.every(tech => tech.role !== role)
    ){

      graceMoves.push({
        mainIndex: nxt.idx[0],
        graceIndex: cur.idx[0],
        role,
        newBar: grace.bar,
        addLen: graceSlot / WHOLE
      });

      removeIdx.add(cur.idx[0]);

      /* 본음에 *N이 붙었다고 표시해 뒤 단계가 또 안 얹게 */
      addTech.push({ index: nxt.idx[0], role });
  ...
  const planned = new Set(
    addTech.map(a => a.index)
  );

  const willPost = index =>
    planned.has(index) || hasPost(notes[index]);
  ...
  if(set.staccato){
    ...
    for(let g = 0; g < groups.length; g++){
      ...
      if(cur.idx.some(i => removeIdx.has(i))){
        continue;
      }
      ...
      const fullDur =
        Math.max(...cur.idx.map(i => lenOf(notes[i])));

      const slot =
        nxt ? nxt.start - cur.start : fullDur;

      if(slot < MIN_TICK * 2){
        continue;
      }

      for(const i of cur.idx){

        if(onlyIdx && !onlyIdx.has(i)){
          continue;
        }

        const dur = lenOf(notes[i]);

        if(
          dur <= slot * 0.55 &&
          !willPost(i) &&
          set.staccato.charAt(0) === "*"
        ){
          addTech.push({ index: i, role: "staccato" });
          planned.add(i);
        }
  ...
  /* ── ③ 악센트 (주변 세기 중앙값보다 3 이상 큼, 0~15 눈금) ── */
  if(set.accent){

    const velOf = n =>
      n.vel === null || n.vel === undefined ? null : n.vel;
    ...
    for(let g = 0; g < groups.length; g++){
      ...
      for(
        let k = Math.max(0, g - 6);
        k <= Math.min(groups.length - 1, g + 6);
        k++
      ){
        ...
        if(vs.length){
          around.push(Math.max(...vs));
        }

      }

      if(around.length < 4){
        continue;   // 근거가 너무 적으면 판단하지 않는다
      }

      around.sort((a, b) => a - b);

      const median =
        around[Math.floor(around.length / 2)];
      ...
      for(const i of cur.idx){
        ...
        if(
          ...
          addTech.push({ index: i, role: "accent" });
          planned.add(i);
        }
  ...
  const graceCount = graceMoves.length;
  const staccatoCount =
    addTech.filter(a => a.role === "staccato").length;
  const accentCount =
    addTech.filter(a => a.role === "accent").length;

  if(!addTech.length && !graceMoves.length){
    showToast(t("tech.autoNone"));
    return;
  }
  ...
  for(const a of addTech){

    const note = notes[a.index];
    const code = set[a.role];

    if(!code){
      continue;
    }

    note.techs.push({ role: a.role, code });

  }

  for(const m of graceMoves){

    const main = notes[m.mainIndex];

    main.bar = m.newBar;
    main.len = main.len + m.addLen;
    main.lengthBase = null;
    main.dotted = false;

  }

  /* 지울 노트는 큰 번호부터 — 앞을 지우면 뒷번호가 밀린다 */
  const doomed =
  ...
  showToast(
    t("tech.autoDone")
      .replace("{s}", staccatoCount)
      .replace("{a}", accentCount)
      .replace("{g}", graceCount)
  );
...
/* ============================================================
   보이는 범위만 그린다 (가상화)
   ============================================================
   6천 음짜리 곡을 재 보니 노트 하나를 고칠 때마다 그 트랙 2,200개를
   전부 다시 만들어 566ms가 걸렸다. 화면에 보이는 건 그중 100~200개
   뿐이다. 그래서 화면 좌우로 세 마디 여유를 두고 그 안의 노트만
   만든다. 스크롤하면 새 범위를 그린다(마디 단위로 묶어 스크롤마다
   그리지 않는다).

   고르기·복사·삭제는 모델(tracks[].notes)로 일하므로 화면 밖 노트도
   전부 대상이 된다. 화면에 없다는 건 그리지 않았다는 뜻일 뿐이다.

   판이 아직 배치되지 않아 폭이 0이면(시험 환경 등) 전부 그린다.
   ============================================================ */
const VIEW_MARGIN_BARS = 3;
...
function visibleBarRange(){
  ...
  const from =
    Math.floor(board.scrollLeft / BAR_W) - VIEW_MARGIN_BARS;

  const to =
    Math.ceil((board.scrollLeft + width) / BAR_W) + VIEW_MARGIN_BARS;
    ...
    if(cached && cached.fingerprint === fingerprint){
      ...
      track.notes.forEach(
        (note,index)=>{
          ...
          /* 화면 밖이면 만들지 않는다 (막대 길이는 이음줄까지 포함) */
          if(view){

            const drawn =
              layout.length.get(index) !== undefined
                ? layout.length.get(index)
                : note.len;

            if(
              note.bar + drawn < view.from ||
              note.bar > view.to
            ){
              return;
            }
...
/* ── 스크롤 지름길 ──
   지문 비교는 "그리기"는 아끼지만 "지문 계산"(트랙마다 노트 전체를
   JSON으로 직렬화)은 매번 한다. 재생 따라가기가 판을 미는 동안에는
   스크롤이 매 프레임 오는데, 마디 창(view)이 같은 마디 안에 머무는
   프레임이 대부분이다 — 그 프레임은 직렬화조차 하지 않고 돌려보낸다.

   lastViewKey는 renderNotes가 끝날 때마다 스스로 갱신하므로,
   편집으로 인한 renderNotes 직접 호출과는 서로 방해하지 않는다. */
...
/* ============================================================
   단축키 배치표 (키맵)
   ============================================================
   어느 키가 어느 일을 부르는지를 표 하나로 모아 둔다.
   단축키 설정 창에서 이 표를 바꿀 수 있고, 바꾼 내용은
   localStorage에 남아 다음에 열 때도 그대로다.

   조합은 "Ctrl+Shift+B" 같은 글자열로 적는다. 수식키 순서는
   항상 Ctrl → Alt → Shift 이고, Cmd(맥)는 Ctrl로 통일한다.
   Esc는 상황마다 하는 일이 달라(떠 있는 노트 취소 → 패널 닫기
   → 고르기 끄기 → 재생 정지) 여기 넣지 않고 그대로 둔다. */

const KEYMAP_STORE_KEY =
  "armis-mml-keymap";

const KEY_ACTIONS = [
  { id:"undo",          defaults:["Ctrl+Z"],                          run:()=>undo() },
  { id:"redo",          defaults:["Ctrl+Y","Ctrl+Shift+Z"],           run:()=>redo() },
  { id:"copy",          defaults:["Ctrl+C"],                          run:()=>copyNotesButton.click() },
  { id:"cut",           defaults:["Ctrl+X"],                          run:()=>cutNotes() },
  { id:"paste",         defaults:["Ctrl+V"],                          run:()=>pasteNotesButton.click() },

  /* Delete/Backspace는 고른 게 없으면 소비하지 않는다 —
     브라우저의 Backspace 동작을 괜히 막지 않기 위해서다. */
  { id:"delete",        defaults:["Delete","Backspace","Alt+Shift+T"],
                        needsSelection:true,                          run:()=>noteDeleteButton.click() },

  { id:"selectAll",     defaults:["Ctrl+A"],                          run:()=>selectAllNotes() },
  { id:"pickMode",      defaults:["Ctrl+S"],                          run:()=>setPickMode(!pickMode) },
  { id:"insertBars",    defaults:["Ctrl+B"],                          run:()=>askInsertBars() },
  { id:"removeBars",    defaults:["Ctrl+Shift+B"],                    run:()=>askRemoveBars() },
  { id:"cutAtPlayhead", defaults:["Ctrl+K"],                          run:()=>cutAtPlayhead() },

  { id:"octaveUp",      defaults:["Shift+ArrowUp"],                   run:()=>shiftSelectedOctave(1) },
  { id:"octaveDown",    defaults:["Shift+ArrowDown"],                 run:()=>shiftSelectedOctave(-1) },
  { id:"velocityUp",    defaults:["Alt+ArrowUp"],                     run:()=>shiftSelectedVelocity(1) },
  { id:"velocityDown",  defaults:["Alt+ArrowDown"],                   run:()=>shiftSelectedVelocity(-1) },

  /* SNAP(1/96마디)은 그리기용 최소 눈금이라 재생 머리를 옮기기엔
     너무 잘다. 한 번에 한 박씩 간다. */
  { id:"stepLeft",      defaults:["ArrowLeft"],
                        run:()=>setPlayheadBar(playheadBar - 0.25, { scrollIntoView:true }) },
  { id:"stepRight",     defaults:["ArrowRight"],
                        run:()=>setPlayheadBar(playheadBar + 0.25, { scrollIntoView:true }) },
  { id:"barPrev",       defaults:["Ctrl+ArrowLeft"],                  run:()=>jumpBars(-1) },
  { id:"barNext",       defaults:["Ctrl+ArrowRight"],                 run:()=>jumpBars(1) },

  /* Home/F3은 마비꼬의 버릇을 따른 것 */
  { id:"goStart",       defaults:["Home","F3"],
                        run:()=>setPlayheadBar(0, { scrollIntoView:true }) },
  { id:"goEnd",         defaults:["End"],                             run:()=>jumpToSongEnd() },

  /* 마비꼬는 F5로 재생한다. 웹에서는 스페이스가 더 익숙해 둘 다 둔다. */
  { id:"play",          defaults:["F5","Space"],                      run:()=>togglePlayback() },

  /* 마비꼬는 Ctrl+T를 쓰지만, 브라우저에서 Ctrl+T는 새 탭이라
     가로챌 수가 없다. 그래서 Alt로 옮겨 두었다. */
  { id:"addTrack",      defaults:["Alt+T"],                           run:()=>trackAddButton.click() }
];


/* 저장된 사용자 지정 키를 읽는다. 표에 없는 항목이나 형식이
   깨진 값은 조용히 버린다 — 예전 판이 남긴 찌꺼기일 수 있다. */
let customKeymap = {};

try{

  const raw =
    localStorage.getItem(KEYMAP_STORE_KEY);
...
function combosOf(actionId){

  if(customKeymap[actionId]){
    return customKeymap[actionId];
  }
...
/* 조합 → 일 찾아보기용 지도. 키맵이 바뀔 때마다 다시 짠다. */
let comboIndex = new Map();

function rebuildComboIndex(){
...
function saveKeymap(){
  ...
  try{
    ...
    if(Object.keys(customKeymap).length){
...
/* 키보드 이벤트를 "Ctrl+Shift+B" 꼴의 조합 글자열로 바꾼다.
   수식키만 눌린 상태면 null. */
function comboFromEvent(event){
  ...
  /* 맥에서는 Option+T의 event.key가 "†"로 들어와 글자 비교가
     빗나간다. Alt가 눌린 동안에는 물리 키(code)로 글자를 본다. */
  if(
    event.altKey &&
    /^Key[A-Z]$/.test(code)
  ){
...
document.addEventListener(
  ...
  event=>{
    ...
    /* 단축키 설정 창이 떠 있는 동안에는 편집 단축키가 쉰다 —
       키를 바꿔 다는 중에 그 키가 진짜로 발동하면 곤란하다. */
    if(keysDialogOpen()){
...
function buildTrackGroups(track){
  ...
  for(const g of groups){
    ...
    /* 게임 한 트랙의 화음 상한을 넘으면 그 뒤 음들이 밀리므로
       여기서 줄여서 내보낸다 (midi-engine.js의 CHORD_MAX 참고).
       롤의 노트 데이터 자체는 건드리지 않는다 — 출력만 줄인다. */
    g.notes =
      trimChordNotes(g.notes);
...
/* ============================================================
   트랙 MML 캐시
   ============================================================
   출력 줄은 편집 한 번마다 모든 트랙의 MML을 다시 만든다. 트랙이 셋에
   음이 2천 개씩이면 그것만 100ms다. 한 번의 편집으로 바뀌는 트랙은
   대개 하나이므로, 트랙의 내용(노트·마커·이름·악기)을 글자로 굳힌
   지문이 같으면 지난 결과를 그대로 쓴다. 노트 층 캐시와 같은 수법이다.
   ============================================================ */
const trackMmlCache = new Map();   // track 객체 → { fingerprint, mml }

function trackMmlFingerprint(track){

  return [
    JSON.stringify(track.notes),
    JSON.stringify(track.velocityMarkers || []),
    JSON.stringify(track.tempoMarkers || []),
    track.name,
    tempo,
    OPT_LENGTH ? 1 : 0
  ].join("|");
...
function generateTrackMML(track){
  ...
  const fingerprint =
    trackMmlFingerprint(track);

  const cached =
    trackMmlCache.get(track);
  ...
  const mml =
    generateTrackMMLUncached(track);

  trackMmlCache.set(track, { fingerprint, mml });

  /* 사라진 트랙의 항목은 걷는다 */
  if(trackMmlCache.size > tracks.length + 8){

    for(const key of [...trackMmlCache.keys()]){

      if(!tracks.includes(key)){
        trackMmlCache.delete(key);
      }
...
function generateTrackMMLUncached(track){
...
let clipboardOrigin = null;        // 복사한 원위치 { bar, row, track } — 다른 트랙에 같은 자리로 붙일 때 씀
  ...
  ()=>{
    ...
    if(
      ...
      if(copySelectedMarker()){
        ...
        /* "마지막으로 복사한 것이 붙는다"를 지키기 위해 노트
           클립보드를 비운다. 안 비우면 붙여넣기가 노트 쪽을
           우선해서, 마커 대신 예전에 복사한 노트가 나온다. */
        clipboard = [];
    ...
    /* 어느 트랙의 어느 자리에서 복사했는지 기억한다.
       다른 트랙에 붙여넣을 때는 화면 가운데에 띄우지 않고
       이 자리에 그대로 놓는다 — 파트를 겹쳐 만들 때의 기본 동작이다. */
    clipboardOrigin = {
      bar: baseBar,
      row: baseRow,
      track: selectedTrack
    ...
    /* 반대 방향도 같은 원리 — 노트를 새로 복사했으면
       예전에 복사해 둔 마커는 잊는다 */
    markerClipboard = null;
...
pasteNotesButton.addEventListener(
  ...
  ()=>{
    ...
    /* 다른 트랙에 붙여넣기 = 같은 자리에 바로 놓기.
       (같은 트랙이면 원위치가 원본과 겹치므로 예전처럼 띄워서 끌게 둔다)
       자리가 막혀 있으면 commitGhost가 거부하고 그 자리에 띄워 둔
       채로 남으니, 그때는 끌어서 놓으면 된다. */
    if(
      clipboardOrigin &&
      clipboardOrigin.track !== selectedTrack
    ){

      startGhost(
        items,
        {
          bar: clipboardOrigin.bar,
          row: clipboardOrigin.row
        }
      );
...
function renderOutputs(){
  ...
  shown.forEach(
    ({ track, index })=>{
      ...
      /* 게임이 세는 방식(음표+쉼표 개수)으로 보여 준다. 문자열 길이는
         참고용으로 툴팁에만 둔다. 3000을 넘으면 붙여넣어도 뒤가
         잘리므로 빨갛게 알린다. */
      const count =
        gameCharCount(field.value);

      const over =
        count > GAME_CHAR_LIMIT;
      ...
      len.title =
        t(
          over ? "out.charsOverTip" : "out.charsTip",
          {
            n: count,
            max: GAME_CHAR_LIMIT,
            raw: field.value.length
          }
        );
  ...
  /* 공동 작업 중이면, 방금의 편집을 방에도 알린다 (collab.js).
     되돌리기(restoringHistory)도 보내야 한다 — 내 화면만 되돌아가고
     방은 그대로면 서로 어긋난다. collab.js가 알아서 차이만 보낸다. */
  if(typeof collabOnLocalChange === "function"){
...
/* 변환 방식 세 갈래 선택 창 — 합주 / 성부 나눔 / 솔로.
   확인창(showConfirm)은 단추가 둘뿐이라 여기서만 쓰는 창을 그때그때
   만들어 띄운다. 생김새는 기존 창들과 같은 dialog 클래스를 쓴다. */
function showMidiModeChoice(){
  ...
  return new Promise(resolve=>{
    ...
    eyebrow.textContent = "midi";

    const title =
      document.createElement("h3");

    title.className = "dialog-title";
    title.textContent = t("midi.modeTitle");

    const text =
      document.createElement("p");

    text.className = "dialog-text";
    text.style.whiteSpace = "pre-line";   // 항목별 줄바꿈이 문구에 들어 있다
    text.textContent = t("midi.modeText2");
    ...
    [
      { label: t("midi.modeEnsemble"), mode: "ensemble", primary: false },
      { label: t("midi.modeVoices"),   mode: "voices",   primary: true  },
      { label: t("midi.modeSolo"),     mode: "solo",     primary: false }
    ].forEach(item=>{
      ...
      button.className =
        "dialog-btn" + (item.primary ? " primary" : "");
...
async function convertOnServer(buffer, options){
  ...
  const url =
    MIDI_API_URL +
    (
      options && options.mode && options.mode !== "ensemble"
        ? (MIDI_API_URL.includes("?") ? "&" : "?") + "mode=" + encodeURIComponent(options.mode) +
          (options.fitTracks ? "&fit=" + encodeURIComponent(options.fitTracks) : "")
        : ""
    );
  ...
  /* 솔로 분배는 서버에서도 1~2초 걸린다. 합주보다 길게 기다린다. */
  const timer =
    setTimeout(
      ()=>controller.abort(),
      options && options.mode === "solo"
        ? MIDI_API_TIMEOUT_MS * 5
        : MIDI_API_TIMEOUT_MS
    );
...
/* mode: "ensemble"(파트 그대로) | "solo"(파트를 합쳐 세 트랙으로 고르게)

   솔로 모드는 세 트랙의 글자 수를 맞추느라 변환을 여러 번 돌려서
   1~2초의 CPU가 든다. 사양이 낮은 폰에서는 이게 버겁다. 그래서
   먼저 서버에 맡긴다 — Cloudflare 유료 플랜(월 $5)이면 요청당 CPU가
   30초라 넉넉하다. 무료 플랜이면 서버가 CPU 한도(10ms)에서 바로
   실패하고, 그때는 브라우저 워커에서 돌린다. 어느 쪽이든 결과는
   같은 엔진이라 똑같다. */
async function convertMidi(buffer, modeOrOptions){

  /* 예전처럼 문자열 하나만 받아도 되고, 옵션 묶음을 받아도 된다 */
  const options =
    typeof modeOrOptions === "object" && modeOrOptions
      ? { mode: "ensemble", ...modeOrOptions }
      : { mode: modeOrOptions || "ensemble" };

  const fromServer =
    await convertOnServer(buffer, options);

  if(fromServer){
    return fromServer;
  }

  return convertLocally(buffer, options);

}

/* convert(buffer)와 같은 결과를 반환하되, 가능하면 브라우저 워커에서 계산한다 */
function convertLocally(buffer, options){
  ...
  return new Promise(
    (resolve, reject)=>{
      ...
      try{
        ...
        try{
          resolve(convert(buffer, options));
        }catch(fallbackError){
...
/* MIDI 바이트 → 편집기 트랙. 파일 선택과 오디오 AI 변환이 같이 쓴다.
   실패하면 throw — 상태 표시는 부르는 쪽 몫이다. */
async function importMidiFromBuffer(buffer, displayName, fixedOptions){

    {

      /* 어떻게 변환할지 묻는다 — 합주(그대로) / 서스테인 / 솔로.
         단, 부르는 쪽이 방식을 정해 준 경우(AI 채보)에는 묻지 않는다. */
      const options =
        fixedOptions ||
        { mode: await showMidiModeChoice() };

      setFileStatus(
        t("midi.reading", { name: displayName }),
        "busy"
      );

      const song =
        await convertMidi(buffer, options);
      ...
      if(!loaded.length){
        ...
        setFileStatus(
          t("midi.noTracks", { name: displayName }),
          "bad"
        );
...
/* ============================================================
   음역별 볼륨 조정 (낮은음 내리기 / 높은음 올리기)
   ============================================================
   베이스가 웅웅거려 멜로디를 덮거나, 반대로 높은 멜로디가 묻힐 때
   해당 음역의 세기를 한꺼번에 손본다. 게임에서 실제로 들리는 소리를
   다듬는 손질이라 미리듣기가 아니라 음표 자체(v 값)를 바꾼다.

   세기는 두 곳에서 온다 — 음마다 붙는 값(note.vel)과, 없으면 구간
   마커 값이다(noteVelocity). 그래서 손보는 순간 그 음에 값을 박아
   둔다. 안 그러면 마커를 나중에 바꿀 때 손질이 없던 일이 된다. */

const lowVolumeButton =
  document.getElementById("lowVolumeButton");

const highVolumeButton =
  document.getElementById("highVolumeButton");

/* 이 음이 손볼 음역에 드는가 */
function inOctaveRange(note, octave, isHigh){

  const oct =
    keyToOctave(rowToKey(note.row));
...
/* 옥타브·크기를 고르는 창. 고르면 { octave, amount }, 취소면 null.
   isHigh면 "이 옥타브 이상 올리기", 아니면 "이하 내리기". */
function showOctaveVolumeDialog(isHigh){

  return new Promise(resolve=>{

    const key = suffix =>
      (isHigh ? "highVol." : "lowVol.") + suffix;
    ...
    text.className = "dialog-text";
    text.textContent = t(key("text"));

    /* 옥타브 고르기 — 화면에 있는 범위 그대로 */
    const octRow =
      document.createElement("div");

    octRow.className = "low-vol-row";

    const octLabel =
      document.createElement("label");

    octLabel.textContent = t(key("octave"));

    const octSelect =
      document.createElement("select");

    for(let o = OCT_BOTTOM; o <= OCT_TOP; o++){

      const opt =
        document.createElement("option");

      opt.value = String(o);
      opt.textContent = t(key("octaveItem"), { o });
    ...
    amtRow.className = "low-vol-row";

    const amtLabel =
      document.createElement("label");

    amtLabel.textContent = t(key("amount"));
    ...
    for(let a = 1; a <= 8; a++){
      ...
      opt.value = String(a);
      opt.textContent = t(key("amountItem"), { n: a });
    ...
    amtRow.appendChild(amtSelect);

    const hint =
      document.createElement("p");

    hint.className = "low-vol-hint";
    ...
    const refreshHint = ()=>{
      ...
      if(track){
        ...
        for(const note of track.notes){
          ...
          if(inOctaveRange(note, octave, isHigh)){
            count++;
          }
      ...
      hint.textContent =
        t("lowVol.hint", { n: count });

    };

    refreshHint();
    octSelect.addEventListener("change", refreshHint);
    ...
    ok.textContent = t("lowVol.apply");

    ok.addEventListener(
      "click",
      ()=>close({
        octave: Number(octSelect.value),
        amount: Number(amtSelect.value)
      })
    );
...
async function runOctaveVolume(isHigh){
  ...
  const choice =
    await showOctaveVolumeDialog(isHigh);
  ...
  const targets =
    track.notes.filter(
      note => inOctaveRange(note, choice.octave, isHigh)
    );

  if(!targets.length){

    showToast(
      t(isHigh ? "highVol.none" : "lowVol.none")
    );
  ...
  for(const note of targets){
    ...
    /* 올릴 때는 게임 상한(v15)을 넘지 않게, 내릴 때는 0 아래로 안 가게 */
    const after =
      isHigh
        ? Math.min(VEL_MAX, before + choice.amount)
        : Math.max(0, before - choice.amount);

    /* 값을 음에 박아 둔다 — 구간 마커에 기대면 나중에 마커를
       바꿀 때 이 손질이 없던 일이 된다. */
    note.vel = after;
  ...
  showToast(
    t(isHigh ? "highVol.done" : "lowVol.done", { n: targets.length })
  );
...
if(lowVolumeButton){
  ...
  lowVolumeButton.addEventListener(
    "click",
    ()=>runOctaveVolume(false)
  );
...
if(highVolumeButton){
  ...
  highVolumeButton.addEventListener(
    "click",
    ()=>runOctaveVolume(true)
  );
...
/* ============================================================
   오디오 AI 변환 (transcribe.js)
   ============================================================
   소리 파일을 AI가 듣고 노트로 옮긴 뒤, 표준 MIDI 바이트로 조립해
   위의 importMidiFromBuffer로 넘긴다 — 그 뒤는 MIDI 불러오기와 같다
   (합주/서스테인/솔로 선택 포함). */

const audioAiButton =
  document.getElementById("audioAiButton");

const audioInput =
  document.getElementById("audioInput");

/* 어떤 녹음인지 — 피아노 단독(지금) / 합주·노래(다음 업데이트) */
function showAudioModeChoice(){
  ...
  return new Promise(resolve=>{
    ...
    eyebrow.className = "dialog-eyebrow";
    eyebrow.textContent = "audio ai";

    const title =
      document.createElement("h3");

    title.className = "dialog-title";
    title.textContent = t("audio.modeTitle");

    const text =
      document.createElement("p");

    text.className = "dialog-text";
    text.textContent = t("audio.modeText");
    ...
    piano.type = "button";
    piano.className = "dialog-btn primary";
    piano.textContent = t("audio.modePiano");
    piano.addEventListener("click", ()=>close("piano"));

    const ensemble =
      document.createElement("button");

    ensemble.type = "button";
    ensemble.className = "dialog-btn";
    ensemble.textContent = t("audio.modeEnsemble");
    ensemble.disabled = true;
    ensemble.style.opacity = ".55";

    actions.appendChild(piano);
    actions.appendChild(ensemble);
...
/* ── 파란 진행창 ── */
let transcribePanel = null;

function showTranscribePanel(name, onCancel){
  ...
  panel.innerHTML =
    '<div class="tp-head">' +
      ...
      '<button type="button" class="tp-cancel"></button>' +
    '</div>' +
    '<div class="tp-stage"></div>' +
    '<div class="tp-rail"><div class="tp-fill"></div></div>' +
    '<div class="tp-foot">' +
      '<span class="tp-percent"></span>' +
      '<span class="tp-eta"></span>' +
    '</div>' +
    '<div class="tp-note"></div>';

  panel.querySelector(".tp-name").textContent = name;
  panel.querySelector(".tp-cancel").textContent = t("audio.cancel");

  panel
    .querySelector(".tp-cancel")
  ...
  transcribePanel = panel;

  return panel;

}

function updateTranscribePanel(m){
  ...
  const stage =
    transcribePanel.querySelector(".tp-stage");

  const rail =
    transcribePanel.querySelector(".tp-rail");

  const fill =
    transcribePanel.querySelector(".tp-fill");

  const percent =
    transcribePanel.querySelector(".tp-percent");

  const eta =
    transcribePanel.querySelector(".tp-eta");

  /* 얼마나 왔는지 모르는 단계는 막대를 흐르게 둔다 */
  const busy = text=>{
  ...
  /* 비율을 아는 단계는 막대를 채우고 %와 남은 시간을 보여 준다 */
  const ratioBar = (text, done, total, seconds)=>{
    ...
    percent.textContent = Math.round(r * 100) + "%";

    eta.textContent =
      seconds > 0
        ? t("audio.eta", { s: seconds })
        : "";
  ...
  if(m.stage === "fallback"){
    ...
    /* 서버를 못 써서 기기 채보로 넘어간다는 안내 한 줄.
       이후 단계들이 진행돼도 이 줄은 남아 있는다 */
    const note =
      transcribePanel.querySelector(".tp-note");

    if(note){
      note.textContent =
        t(m.reason === "busy" ? "audio.fallbackBusy" : "audio.fallbackError");
    }
  ...
  if(m.stage === "server"){
    ...
    /* 서버 채보 — 곡 길이 기반 예상 진행률.
       길이를 못 읽은 파일이면 막대를 흐르게 둔다 */
    if(m.total > 0){
      ratioBar(
        t("audio.stageServer"),
        m.done,
      ...
      busy(t("audio.stageServer"));
    }

  }else if(m.stage === "read"){

    /* 파일 읽기 — 크기를 알면 %로 */
    if(m.total > 0){
      ratioBar(t("audio.stageRead"), m.done, m.total, 0);
    }else{
      busy(t("audio.stageRead"));
    }

  }else if(m.stage === "decode"){

    busy(t("audio.stageDecode"));

  }else if(m.stage === "resample"){

    busy(t("audio.stageResample"));

  }else if(m.stage === "runtime"){

    busy(t("audio.stageRuntime"));

  }else if(m.stage === "runtimeReady"){

    busy(t("audio.stageModelPrep"));

  }else if(m.stage === "cache"){

    busy(t("audio.stageCache"));

  }else if(m.stage === "model"){

    /* 모델 내려받기 — 총량을 알므로 %와 남은 시간을 낸다 */
    if(!transcribePanel.__modelStart){
      transcribePanel.__modelStart = Date.now();
    }
    ...
    const elapsed =
      (Date.now() - transcribePanel.__modelStart) / 1000;

    const left =
      elapsed > 1 && done > 0 && total > done
        ? Math.round((total - done) / (done / elapsed))
        : 0;

    ratioBar(
      t("audio.stageModel", { mb: Math.round(done / 1e6) }),
      done,
      total,
      left
    );

  }else if(m.stage === "session"){

    busy(t("audio.stageSession"));

  }else if(m.stage === "transcribe"){

    ratioBar(
      t("audio.stageTranscribe", { done: m.done, total: m.total }),
      m.done,
      m.total,
      m.extra || 0
    );

  }else if(m.stage === "post"){

    busy(t("audio.stagePost"));

  }else if(m.stage === "build"){

    busy(t("audio.stageBuild"));
...
function hideTranscribePanel(){
  ...
  const panel = transcribePanel;

  transcribePanel = null;
...
if(audioAiButton && audioInput){

  audioAiButton.addEventListener(
    "click",
    ()=>audioInput.click()
  );
  ...
  audioInput.addEventListener(
    ...
    async()=>{
      ...
      /* 채보 대상은 피아노 솔로로 고정한다. 변환 방식도 묻지 않는다 —
         AI가 뽑은 노트는 한 사람이 연주할 3트랙 피아노 악보로 만든다
         (서스테인으로 울림을 살리고, 넘치면 편곡기가 다듬는다). */
      ...
      try{
        ...
        await importMidiFromBuffer(
          midiBytes.buffer,
          file.name,
          { mode: "voices", fitTracks: MAX_TRACKS }
        );
        ...
        setFileStatus(
          error && error.code === "no-model"
            ? t("audio.noModel")
            : error && error.code === "no-runtime"
            ? t("audio.noRuntime")
            : error && error.code === "badModel"
            ? t("audio.brokenModel") +
...
midiInput.addEventListener(
  ...
  async()=>{
    ...
    try{
      ...
      await importMidiFromBuffer(buffer, file.name);
...
function playOne(
  ...
  /* v0~v15를 소리 크기로. 제곱을 쓰면 사람 귀에 고르게 들린다.
     ...
     0.42는 "너무 크다"는 피드백으로 0.30으로 내렸다 (0.22는 예전에
     인게임 녹음보다 한참 작았음). master에 리미터가 있으므로 여러 음이
     겹쳐도 깨지지 않는다. */
  let level =
    Math.pow(item.velocity / VEL_MAX, 2) * 0.30 * techGain;

  /* 음역별 세기 보정(500Hz 아래 옥타브당 +1.6dB, 위는 내림)이 여기
     있었지만 걷어냈다 — 낮은 음을 키우고 높은 음을 깎는 게 "미디
     원본과 소리가 다르다"는 피드백을 받았다. 이제 세기는 오로지
     v 값과 악기 음색이 정한다. */
  ...
  if(item.percussion){
    ...
    /* 낮은 음 배음 보강(bassLift)이 여기 있었지만 걷어냈다 — 300Hz
       아래 음에 위쪽 배음(×2~×5)을 얹어 저음을 또렷하게 만드는
       처리였는데, "미디 원본과 소리가 다르다"는 피드백으로 뺐다.
       악기 음색이 원래 갖고 있는 배음(voice.partial)은 보정이 아니라
       음색 그 자체이므로 그대로 둔다. */
...
async function startPlayback(){
  ...
  const pump = ()=>{
    ...
    /* 곡이 끝났는지는 여기서도 본다.
       ...
       출력 지연만큼은 더 기다린다 — 시계로는 끝났어도 마지막 음이
       아직 스피커로 가는 중일 수 있고, 여기서 정리하면 뚝 끊긴다. */
    const tailLatency =
      (ctxNow.outputLatency || 0) +
      (ctxNow.baseLatency || 0);

    if(elapsed > playback.endsAt + 0.3 + tailLatency){
  ...
  const follow = ()=>{
    ...
    /* 오디오 시계(currentTime)는 "지금 만들고 있는" 샘플의 시각이고,
       귀에는 출력 지연(장치·버퍼)만큼 늦게 도착한다. 블루투스 이어폰이나
       방금 깨어난 폰 스피커에서는 이게 0.2~0.5초나 된다. 지연을 빼지
       않으면 노란 선이 소리보다 그만큼 앞서 달려, 선이 가리키는 노트와
       들리는 노트가 어긋난다. 재생 첫머리의 짧은 무음 동안에도 선이
       (음수 → 0으로 잘려) 제자리에 서서 기다리게 된다. */
    const heardLatency =
      (playback.ctx.outputLatency || 0) +
      (playback.ctx.baseLatency || 0);
...
/* ============================================================
   재생 버튼 한 개 — 재생과 일시정지를 오간다
   ============================================================
   정지 버튼을 없앴다. 손가락으로 쓰는 화면에서는 버튼이 적을수록
   낫고, 재생·멈춤을 오가는 일이 대부분이기 때문이다.

     울리지 않을 때 누름 : 재생 머리 자리에서 재생
     울리는 중에 누름     : 일시정지 — 재생 머리를 그 자리에 둔다
     다시 누름            : 거기서 이어서 재생

   처음 자리로 돌아가려면 Esc(정지) 나 Home 을 쓴다. 재생 머리를
   다른 데로 옮겨 놓고 누르면 당연히 그 자리에서 시작한다.

   버튼 얼굴은 "다음에 누르면 무엇이 되는지"다 — 울리는 중이면
   일시정지 막대, 아니면 재생 삼각형. 모양은 styles.css의 .glyph가
   그린다(글자를 쓰면 사파리가 컬러 이모지로 바꿔 버린다).
   ============================================================ */
/* ============================================================
   재생 시간 표시
   ============================================================
...
/* 재생 버튼과 Space/F5 가 같이 쓰는 한 가지 동작 */
...
/* 재생 머리를 곡의 끝(마지막 노트가 끝나는 자리)으로 보낸다.
   노트가 하나도 없으면 songEndBar가 0이라 맨 앞과 같다. */
function jumpToSongEnd(){
...
if(barStartButton){
  ...
  barStartButton.addEventListener(
    "click",
    ()=>setPlayheadBar(
      0,
      { scrollIntoView:true }
...
/* 버튼 ↔ 키맵의 일 이름. 툴팁 끝에 붙일 키는 여기 적힌
   이름으로 키맵에서 그때그때 찾아온다 — 그래서 단축키를
   바꿔 달아도 툴팁이 저절로 따라온다. */
const SHORTCUT_HINTS = [
  ['[data-icon="undo"]',        ["undo"]],
  ['[data-icon="redo"]',        ["redo"]],
  ['[data-icon="note-delete"]', ["delete"]],
  ['#selectMode',               ["pickMode","selectAll"]],
  ['#copyNotes',                ["copy","cut"]],
  ['#pasteNotes',               ["paste"]],
  ['[data-icon="play"]',        ["play"]],
  ['#barPrev',                  ["barPrev"]],
  ...
  ['#barStart',                 ["goStart"]],
  ['#barEnd',                   ["goEnd"]],
  ['#insertBars',               ["insertBars"]],
  ['#removeBars',               ["removeBars"]],
  ['[data-icon="track-add"]',   ["addTrack"]],
  ['#cutNotes',                 ["cutAtPlayhead"]]
];


function applyShortcutHints(){

  for(const [selector, actionIds] of SHORTCUT_HINTS){
    ...
    /* 말이 바뀌든 키가 바뀌든 언제나 "설명  (키)" 꼴로 다시 만든다.
       덧붙이기만 하면 부를 때마다 키가 겹겹이 쌓인다. */
    const titleKey =
...
/* ============================================================
   단축키 설정 창
   ============================================================
   줄의 키 부분을 누르면 "키를 누르세요…" 상태가 되고, 다음에
   누른 조합이 그 일에 배정된다(그 일의 키는 그 하나로 바뀐다).
   다른 일이 이미 쓰고 있는 조합이면 그쪽에서 떼어 온다. */

const keysBackdrop =
  document.getElementById("keysBackdrop");

const keysListEl =
  document.getElementById("keysList");

const keysOpenButton =
  document.getElementById("keysOpen");

const keysCloseButton =
  document.getElementById("keysClose");

const keysResetAllButton =
  document.getElementById("keysResetAll");

let keysRecordingAction = null;


function keysDialogOpen(){

  return (
    !!keysBackdrop &&
    keysBackdrop.classList.contains("show")
  );

}


function afterKeymapChange(){
...
function renderKeysList(){
  ...
  for(const action of KEY_ACTIONS){
    ...
    row.appendChild(comboButton);

    /* 기본값에서 바꾼 줄에만 되돌리기 단추를 단다 */
    if(customKeymap[action.id]){
...
function startKeyRecording(actionId, button){
  ...
  /* 붙잡기 단계(capture)에서 받아야 본 편집 단축키보다 먼저 온다 */
  document.addEventListener(
    "keydown",
    recordKeyPress,
    true
...
function openKeysDialog(){
  ...
  keysBackdrop.classList.add("show");

  requestAnimationFrame(
    ()=>keysBackdrop.classList.add("visible")
  );
...
function closeKeysDialog(){
  ...
  keysBackdrop.classList.remove("visible");

  setTimeout(
    ()=>{

      if(
        !keysBackdrop.classList.contains(
          "visible"
        )
      ){

        keysBackdrop.classList.remove("show");
...
if(keysOpenButton){
  ...
  keysOpenButton.addEventListener(
    "click",
    openKeysDialog
  );
...
if(keysCloseButton){
  ...
  keysCloseButton.addEventListener(
    "click",
    closeKeysDialog
  );
...
if(keysResetAllButton){

  keysResetAllButton.addEventListener(
    "click",
    ()=>{
...
if(keysBackdrop){
  ...
  /* 창 밖(어두운 배경)을 누르면 닫힌다 */
  keysBackdrop.addEventListener(
    "click",
    event=>{

      if(event.target === keysBackdrop){
...
/* 오른쪽 패널(세기·기법·템포)은 어떤 화면에서도 항상 제자리에
   있는 기둥이다. 예전에는 좁은 화면에서 "패널" 단추로 여닫는
   서랍이었지만, 폰이 뷰포트 1280 고정으로 데스크톱 화면을 통째
   축소해 보게 되면서 서랍도 단추도 필요가 없어져 걷어냈다. */
