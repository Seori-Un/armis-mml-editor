...
/* ============================================================
   낮은음 볼륨 조정
   ============================================================
   베이스가 웅웅거려 멜로디를 덮을 때, 일정 옥타브 아래 음들의 세기를
   한꺼번에 낮춘다. 게임에서 실제로 들리는 소리를 다듬는 손질이라
   미리듣기가 아니라 음표 자체(v 값)를 바꾼다.

   세기는 두 곳에서 온다 — 음마다 붙는 값(note.vel)과, 없으면 구간
   마커 값이다(noteVelocity). 그래서 낮추는 순간 그 음에 값을 박아
   둔다. 안 그러면 마커를 나중에 바꿀 때 손질이 없던 일이 된다. */

const lowVolumeButton =
  document.getElementById("lowVolumeButton");

/* 옥타브·낮출 양을 고르는 창. 고르면 { octave, amount }, 취소면 null. */
function showLowVolumeDialog(){
  ...
  return new Promise(resolve=>{
    ...
    title.className = "dialog-title";
    title.textContent = t("lowVol.title");

    const text =
      document.createElement("p");

    text.className = "dialog-text";
    text.textContent = t("lowVol.text");
    ...
    octLabel.textContent = t("lowVol.octave");
    ...
    for(let o = OCT_BOTTOM; o <= OCT_TOP; o++){
      ...
      opt.value = String(o);
      opt.textContent = t("lowVol.octaveItem", { o });
    ...
    amtLabel.textContent = t("lowVol.amount");
    ...
    for(let a = 1; a <= 8; a++){
      ...
      opt.value = String(a);
      opt.textContent = t("lowVol.amountItem", { n: a });
    ...
    const refreshHint = ()=>{
      ...
      if(track){
        ...
        for(const note of track.notes){
          ...
          if(keyToOctave(rowToKey(note.row)) <= octave){
            count++;
          }
  ...
  lowVolumeButton.addEventListener(
    ...
    async()=>{
      ...
      const targets =
        track.notes.filter(
          note =>
            keyToOctave(rowToKey(note.row)) <= choice.octave
        );
