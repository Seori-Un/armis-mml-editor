...
function toggleTieFor(indexes){
  ...
  /* 고른 조각들 가운데 "이어받을 짝이 이미 있는" 것만 잇는다.

     · 화음 속 구성음이어도 된다 — MML은 구성음마다 길이와 이음줄을
       따로 적을 수 있다(f8&f8:d+4). 예전에는 "화음 전체를 고르라"며
       통째로 거부했는데, 반주가 같은 자리에 있기만 해도 멜로디 조각을
       못 잇는 벽이 됐다.
     · 새 조각을 만들어 붙이지 않는다 — "고른 것들을 하나로"가 의도인데
       마지막 조각 뒤에 덤 조각이 생기면 곡이 늘어난다. 한 음을 늘리고
       싶으면 그 음 하나만 골라 이으면 된다(그때는 새로 만들어 준다). */
  const toTie =
    notes.filter(
      note =>
        !note.tie &&
        tiePartnerIndex(track, note) >= 0
    );
  ...
  if(!toTie.length){
    ...
    /* 한 자리의 완전한 화음을 골랐고 아직 안 이어져 있다면, 화음을
       ...
       (사슬 잇기가 아니라 "화음 전체 선택 → 잇기 = 늘이기"라는
        본래 기능이다. 구성음 끝이 가지런해야 어디서 이어질지 정해진다) */
    const firstBar =
      notes[0].bar;
    ...
    /* 더 이을 게 없다. 고른 것 안에 이어진 조각이 있다면 이미 완성된
       사슬을 다시 누른 것 — 토글답게 끊어 준다. (사슬의 꼬리는 tie가
       꺼져 있어서 위의 "전부 이어져 있으면 끊기"에 안 걸린다) */
...
function toggleTie(index){
  ...
  /* 짝이 이미 있으면 그냥 잇는다 — 화음 속 구성음이어도 된다.
     MML은 화음에서 구성음마다 길이와 이음줄을 따로 적을 수 있다
     (f8&f8:d+4 처럼). 예전에는 "화음 전체를 고르라"며 막았는데,
     반주가 같은 자리에 있기만 해도 멜로디 조각을 못 잇는 벽이 됐다. */

  /* 짝이 없어서 새로 만들어야 할 때만, 화음 구성음 하나로는 못 만든다 —
     늘어난 조각이 화음의 어느 음 몫인지 정할 수 없기 때문이다. */
  if(partner < 0){
