...
/* mode: "ensemble"(파트 그대로) | "solo"(파트를 합쳐 세 트랙으로 고르게)
   ...
   같은 엔진이라 똑같다. */
async function convertMidi(buffer, mode){
...
async function importMidiFromBuffer(buffer, displayName){

    {

      /* 어떻게 변환할지 묻는다 — 합주(그대로) / 성부 나눔 / 솔로.
         성부 나눔: 트랙 여유가 있으면 겹치는 음을 성부로 갈라
         지속음이 원본처럼 끝까지 울린다 (midi-engine.js allocateVoices).
         피아노곡이 합주 변환에서 마른 스타카토처럼 들리는 문제의 답이다. */
      const mode =
        await showMidiModeChoice();
