...
function applyDragLength(
  ...
  /* 점온음표(1.5마디)까지는 음표 하나로 적을 수 있다. 그 안에서는
     한 음표 길이에만 서게 해서, 손을 놓는 순간 조각으로 갈라지는
     일이 없게 한다. 그보다 길면 어차피 여러 토막이라 예전대로 둔다. */
  const single =
    wantRaw <= MAX_SINGLE_TICK;

  const ceiling =
    single
      ? Math.max(
          MIN_TICK,
          largestSingleTickWithin(
            Math.min(roomTicks, MAX_SINGLE_TICK)
          )
        )
      : (
...
function fitLengthFromEnd(
  ...
  /* 원하는 길이에서 시작해 짧은 쪽으로 훑으며,
     ...
     후보는 뒤쪽 손잡이(applyDragLength)와 똑같이 "음표 하나로 적히는
     길이"만 쓴다. 예전에는 이어붙여야 적히는 길이(108틱 = 4&32)까지
     후보에 넣었다. 눈금이 촘촘해지는 대신, 조금 줄이려고 끌었을
     뿐인데 손을 놓는 순간 음이 두 토막으로 갈라졌다. 앞뒤 손잡이가
     서로 다르게 움직이는 것도 손에 익지 않는다.

     점온음표보다 긴 길이는 어차피 한 음표로 못 적으므로, 그때만
     예전처럼 정확히 적히는 길이를 쓴다 — 그건 정말로 길게 늘린
     것이라 이음줄이 나오는 편이 맞다. */
  const desiredTicks =
...
function startNotePointer(
  ...
  /* 선택 모드(노란 네모)가 켜져 있거나 Ctrl(⌘)·Shift를 누른 채 노트를
     ...
     눌러야만 토글이 되는 것처럼 보였다. */
  /* 손잡이(양 끝의 길이 조절 자리)를 잡았는지 먼저 본다.

     예전에는 선택 모드에서 손잡이를 잡아도 아래 additive 분기로
     들어가 "고른 것 끌어 옮기기"가 되어 버렸다. 그래서 여러 개
     골라 놓고 길이를 바꾸려고 끝을 잡으면 노트가 통째로 이동했다.
     손잡이를 잡았으면 어느 모드든 길이 조절이다 — 고른 것 안에
     없는 노트면 고른 것에 넣은 뒤 함께 조절한다. */
  const grabbedHandle =
  ...
  /* ------------------------------------------------------------
     같이 끌려갈 음들
     ------------------------------------------------------------
     화음 구성음은 하나만 늘이면 시작이나 끝이 어긋나 화음이 깨진다.
     그래서 잡은 음이 고른 것 안에 있으면, 고른 것을 전부 함께 끈다.
     "전부 아니면 안 함"이라 하나라도 막히면 그 순간 아무도 움직이지
     않는다(아래 fitLengthAll / resizeAllFromEnd 참고).

     고르지 않은 음을 잡았을 때는 예전처럼 그 음만 움직인다. */
  const partners =
    multiSelection.has(index)
      ? [...multiSelection]
          .map(other => track.notes[other])
          .filter(Boolean)
      : [note];
  ...
  pointerState = {
    ...
    partnerSpans:
      noteSpans(track, new Set(partners)),
...
function handleNotePointerMove(
  ...
  if(
    ...
    if(state.partners.length > 1){
      ...
      /* 동행 전부가 같은 만큼 늘어나고 줄어든다. 화음이라면 시작이
         같으므로 길이도 계속 같이 간다. 가장 빡빡한 자리에 맞춰
         다 함께 멈춘다 — 하나만 더 가면 화음이 어긋난다. */
      const delta =
        wantLen - state.startLen;

      let limit = Infinity;

      for(const item of state.startState){
    ...
    if(state.partners.length > 1){
      ...
      /* 동행 전부가 같은 만큼 앞으로 자란다. 각자 자기 끝은 붙잡아
         둔 채 시작만 앞당긴다.

         길이는 반드시 "음표로 적을 수 있는 값"이어야 한다. 아무
         값이나 넣으면 손을 놓을 때 splitNoteByTokens가 이음줄로
         쪼개는데, 화음에서 그러면 조각이 화음 밖으로 튀어나가
         d+2:g2g64 같은 엉뚱한 글자가 된다.

         그래서 fitLengthFromEnd로 저마다 들어갈 수 있는 길이를 재고,
         그중 가장 덜 자란 만큼으로 다 함께 맞춘다. 화음은 길이가
         같으므로 시작도 저절로 가지런해진다. */
      const delta =
        (state.startLen - dx / BAR_W) -
        state.startLen;

      let useDelta = Infinity;
...
function handleNotePointerUp(
  ...
  /* 길이를 바꿨다면, 한 음표로 못 적는 길이는 이음줄로 나눠 놓는다.
     ...
     여럿을 함께 끌었으면 전부 나눠야 한다. 잡은 음 하나만 나누면
     화음의 한 구성음만 두 토막이 되어, 그 조각이 화음 밖으로
     튀어나간 별개의 음처럼 적힌다 (d+2:g2g32). 전부 나누면 두
     화음이 이음줄로 이어진 모양이 되고, MML을 만들 때 다시 하나의
     긴 화음으로 합쳐진다. */
  if(
    state.mode !== "move" &&
    tracks[selectedTrack]
    ...
    /* 뒤에서부터 나눠야 아직 처리하지 않은 번호가 밀리지 않는다 */
    const order =
      (state.partners || [])
        .map(note => track.notes.indexOf(note))
        .filter(index => index >= 0)
        .sort((a,b) => b - a);

    for(const index of (order.length ? order : [state.index])){
...
function playOne(
  ...
  let level =
    Math.pow(item.velocity / VEL_MAX, 2) * 0.42;
