...
function updateTranscribePanel(m){
  ...
  if(m.stage === "decode"){
    stage.textContent = t("audio.stageDecode");
    busy();
  }else if(m.stage === "model"){
    stage.textContent =
      t("audio.stageModel", { mb: Math.round((m.done || 0) / 1e6) });
    busy();
  }else if(m.stage === "session"){
    stage.textContent = t("audio.stageSession");
    busy();
    ...
    eta.textContent =
      m.extra > 0
        ? t("audio.eta", { s: m.extra })
        : "";

  }else if(m.stage === "post"){
    stage.textContent = t("audio.stagePost");
    busy();
  }else if(m.stage === "build"){
    stage.textContent = t("audio.stageBuild");
    busy();
  }
