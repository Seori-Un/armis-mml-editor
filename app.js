...
/* ── 가볍게(저사양) 모드 ──
   ...
   켜고 끄기 우선순위:
   ① 사용자가 단추로 정한 값 (localStorage에 남는다)
   ② 정한 적이 없으면 기기 스스로 — 메모리 4GB 이하 또는
      코어 4개 이하면 켠다. deviceMemory는 크롬 계열에만 있고
      사파리에는 없어서, 둘 중 하나만 있어도 본다. */

const LITE_STORE_KEY =
  "armis-mml-lite";

const liteToggleButton =
  document.getElementById("liteToggle");
...
function setLite(on, remember){
  ...
  if(liteToggleButton){
    ...
    liteToggleButton.classList.toggle(
      "on",
      on
    );
...
setLite(detectLite(), false);

if(liteToggleButton){

  liteToggleButton.addEventListener(
    "click",
    ()=>{
