...
/* ============================================================
   Cloudflare Worker — MIDI 변환 API
   ============================================================
   위쪽은 편집기가 쓰는 것과 같은 midi-engine.js 그대로다.
   여기서부터가 서버 진입점이다.

   화면(GitHub Pages)에서 MIDI 파일 바이트를 그대로 POST로 보내면,
   여기서 convert(buffer)를 돌려 만든 song을 JSON으로 돌려준다.
   무거운 파싱/변환 계산이 사용자 기기가 아니라 이 서버에서
   일어나므로, 느린 기기에서 큰 MIDI를 불러올 때 화면이
   멈추지 않는다. */

const ALLOWED_ORIGINS = [
  // 브라우저가 보내는 Origin 헤더는 경로(/armis-mml-editor/) 없이
  // "스킴 + 호스트"까지만이라, 저장소 경로는 빼고 이렇게 적는다.
  "https://seori-un.github.io"
];
...
export default {
  ...
  async fetch(request, env){
    ...
    /* ── 공동 작업 방 ──
       /room/new  : 방을 만들고 6자리 방번호를 돌려준다 (POST, 몸통 = 처음 곡)
       /room/ws   : ?code=번호 로 WebSocket 접속 → 해당 방 Durable Object로 넘긴다

       둘 다 Durable Object 바인딩(COLLAB_ROOM)이 있어야 동작한다.
       wrangler.toml에 바인딩을 안 넣고 배포하면 env.COLLAB_ROOM이 없으므로
       친절한 오류를 돌려준다 — MIDI 변환(아래)은 그와 무관하게 계속 된다. */
    if(url.pathname === "/room/new" || url.pathname === "/room/ws"){

      if(!env || !env.COLLAB_ROOM){

        return new Response(
          JSON.stringify({ ok:false, message:"서버에 공동 작업 기능이 배포되지 않았습니다" }),
          { status:501, headers:{ ...cors, "Content-Type":"application/json" } }
        );

      }

      // WebSocket은 Origin 검사를 여기서 직접 한다 (CORS가 안 걸리므로)
      if(
        ALLOWED_ORIGINS.length > 0 &&
        origin &&
      ...
      if(url.pathname === "/room/new"){
        ...
        return handleRoomNew(request, env, cors);
      ...
      if(!/^[0-9]{6}$/.test(code)){
        ...
        return new Response("잘못된 방번호", { status: 400 });

      }

      const stub =
        env.COLLAB_ROOM.get(
          env.COLLAB_ROOM.idFromName("room:" + code)
        );
    ...
    if(request.method !== "POST"){
      ...
      return new Response(
        JSON.stringify({ ok:false, message:"POST로 MIDI 파일 바이트를 보내세요" }),
        { status:405, headers:{ ...cors, "Content-Type":"application/json" } }
      );

    }

    // MIDI 파일 하나가 이 크기를 넘으면 거절한다.
    // 서버가 아무 크기나 다 받아주면, 이상한 값을 계속 보내는
    // 요청 하나로 무료 사용량을 다 써버릴 수 있다.
    ...
    if(buffer.byteLength === 0){
      ...
      return new Response(
        JSON.stringify({ ok:false, message:"빈 요청입니다" }),
    ...
    if(buffer.byteLength > MAX_BYTES){
      ...
      return new Response(
        JSON.stringify({ ok:false, message:"파일이 너무 큽니다 (8MB 제한)" }),
        { status:413, headers:{ ...cors, "Content-Type":"application/json" } }
      );

    }

    /* ?mode=solo 면 파트를 합쳐 세 트랙으로 나눈다 (midi-engine.js 참고).
       브라우저는 솔로를 서버로 보내지 않지만, 직접 부르는 경우를 위해 받아 둔다. */
    const mode =
      new URL(request.url).searchParams.get("mode") === "solo"
        ? "solo"
        : "ensemble";
...
/* ============================================================
   공동 작업 방 (Durable Object)
   ============================================================
   방 하나 = Durable Object 하나. 방번호(6자리)를 이름으로 쓴다.

   방 안에는
     doc  : 곡 전체 (아래 "문서 모양" 참고)
     seq  : 지금까지 적용한 편집 걸음 수 (늦게 들어온 사람 동기화용)
   이 저장돼 있고, 접속자들의 WebSocket으로 편집(op)을 중계한다.

   문서 모양 — 편집기 쪽 collab.js와 반드시 같아야 한다:
     doc = { tracks: [
       { cid, meta:{ name, midiName, label, velocityMarkers, tempoMarkers },
         notes: { 노트cid: 노트값, ... } }
     ] }
   'selected'(트랙 보임/숨김)는 사람마다 다른 화면 상태라 문서에 없다.

   op 종류 — applyCollabOps()가 정의한다. 이 함수는 collab.js에도
   똑같이 있다. 한쪽을 고치면 반드시 다른 쪽도 고칠 것.

   비용: WebSocket 최대 절전(hibernation) API를 쓰므로 아무도
   편집하지 않는 동안에는 과금 시간이 흐르지 않는다. 7일 동안
   아무 활동이 없으면 alarm이 방을 통째로 지운다.
   ============================================================ */

const ROOM_IDLE_MS = 7 * 24 * 60 * 60 * 1000;   // 7일
const ROOM_MAX_PEERS = 8;
const ROOM_MAX_DOC_BYTES = 4 * 1024 * 1024;      // 곡 하나 4MB면 충분하고도 남는다

const PEER_COLORS = [
  "#e8590c", "#2f9e44", "#1971c2", "#9c36b5",
  "#c2255c", "#0c8599", "#e8b60c", "#846358"
];


/* op 목록을 문서에 적용한다. (편집기 collab.js와 완전히 동일해야 함) */
...
/* 방을 만든다. 몸통 = { doc } (만든 사람의 지금 곡).
   6자리 번호를 무작위로 뽑아 보고, 이미 살아 있는 방이면 다시 뽑는다. */
async function handleRoomNew(request, env, cors){

  if(request.method !== "POST"){

    return new Response(
      JSON.stringify({ ok:false, message:"POST로 보내세요" }),
      { status:405, headers:{ ...cors, "Content-Type":"application/json" } }
    );
  ...
  try{
    ...
    if(raw.length > ROOM_MAX_DOC_BYTES){

      return new Response(
        JSON.stringify({ ok:false, message:"곡이 너무 큽니다" }),
        { status:413, headers:{ ...cors, "Content-Type":"application/json" } }
      );
  ...
  const doc =
    body && body.doc && Array.isArray(body.doc.tracks)
      ? body.doc
      : { tracks: [] };
  ...
  return new Response(
    JSON.stringify({ ok:false, message:"방번호를 만들지 못했습니다. 다시 시도해 주세요." }),
...
export class CollabRoom {
  ...
  constructor(state, env){
    ...
    // 절전에서 깨어날 때마다 저장소에서 다시 읽는다 (메모리는 날아가므로)
    this.doc = null;
    this.seq = 0;
    this.loaded = false;
  ...
  async load(){
    ...
    const stored =
      await this.state.storage.get(["doc", "seq"]);

    const rawDoc =
      stored.get("doc");

    this.doc =
      rawDoc ? JSON.parse(rawDoc) : null;
  ...
  async alarm(){
    ...
    const last =
      (await this.state.storage.get("lastActive")) || 0;

    if(Date.now() - last >= ROOM_IDLE_MS - 1000){
      ...
      await this.state.storage.setAlarm(last + ROOM_IDLE_MS);
  ...
  async fetch(request){
    ...
    // 방 만들기 (worker의 handleRoomNew가 내부적으로 부른다)
    ...
    if(request.headers.get("Upgrade") === "websocket"){
      ...
      if(this.state.getWebSockets().length >= ROOM_MAX_PEERS){
      ...
      const name =
        (url.searchParams.get("name") || "").slice(0, 24) || "익명";
      ...
      const used =
        new Set(
          this.state.getWebSockets().map(
            ws => (this.peerOf(ws) || {}).color
          )
        );

      const color =
        PEER_COLORS.find(c => !used.has(c)) ||
      ...
      this.broadcast(
        { type: "peers", peers: this.peerList() },
        server
      );
  ...
  peerList(){
    ...
    return this.state.getWebSockets().map(
      ws => {
        ...
        return {
          ...
          name: peer.name,
          color: peer.color,
          sel: peer.sel === undefined ? null : peer.sel
        };
  ...
  async webSocketMessage(ws, raw){
    ...
    if(typeof raw !== "string" || raw.length > ROOM_MAX_DOC_BYTES){
      return;
    }
    ...
    if(msg.type === "ops" && Array.isArray(msg.ops)){
      ...
      this.doc =
        applyCollabOps(this.doc, msg.ops);
      ...
      try{
        ...
        ws.send(
          JSON.stringify({ type: "ack", seq: this.seq })
        );
    ...
    if(msg.type === "presence"){
      ...
      /* 그 사람이 지금 어디서 뭘 만지는지.
           sel    : 보고 있는 트랙 cid
           cursor : 재생선(커서) 마디 위치
           notes  : 잡고 있는 노트들의 [cid, bar, row, len] (최대 60개)
         서버는 내용을 저장하지 않고(입장자 목록용 sel만 빼고)
         가볍게 검사해서 그대로 중계만 한다. */
      peer.sel =
        typeof msg.sel === "string" ? msg.sel : null;

      ws.serializeAttachment(peer);

      const cursor =
        Number.isFinite(msg.cursor) ? msg.cursor : null;

      /* mouse = [마디, 줄] — 그 사람 마우스가 판 위 어디에 있는지 */
      const mouse =
        Array.isArray(msg.mouse) &&
        msg.mouse.length === 2 &&
        Number.isFinite(msg.mouse[0]) &&
        Number.isFinite(msg.mouse[1])
          ? msg.mouse
          : null;
  ...
  async webSocketClose(ws){
    ...
    this.broadcast(
      { type: "peers", peers: this.peerList().filter(p => p.id !== (this.peerOf(ws) || {}).id) },
      ws
    );
