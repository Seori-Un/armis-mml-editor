...
async function testTransport(win){
  ...
  const report = await runAsync(win, `
    ...
    const face = () => playButton.textContent.trim();
    ...
    playButton.click();
    await tick();
    const rewound = playheadBar;
    const backToIdle = face();

    playButton.click();
    await tick();
    const playingAgain = !!playback;
    stopPlayback(true);
    ...
    return {
      ...
      rewound, backToIdle, playingAgain, next, prev,
  ...
  check("① 누르면 재생된다", report.started && report.playing === "❚❚");
  check("② 누르면 그 자리에 세운다", report.paused === "■" && report.heldBar === 1);
  check("③ 누르면 시작했던 자리로 돌아간다", report.rewound === 1 && report.backToIdle === "►", "머리 " + report.rewound);
  check("④ 누르면 다시 재생된다", report.playingAgain);
