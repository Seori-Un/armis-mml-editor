...
function testMidiImport(win, midi){
  ...
  const report = run(win, `
    ...
    /* 자동 기법 판정은 마비꼬 방식에 맞춰 기본이 꺼져 있다.
       이 검사는 판정·표시 기능 자체를 확인하므로 켜고 돌린다. */
    AUTO_TECHNIQUE = true;
    const song = convert(buffer);
    AUTO_TECHNIQUE = false;
...
function testCompression(win){
  ...
  const report = run(win, `
    ...
    /* 끝 옮기기(OPT_TIE_SNAP)는 마비꼬 방식에 맞춰 기본이 꺼져 있다.
       이 검사는 그 기능 자체를 확인하므로 켜고 돌린다. */
    OPT_TIE_SNAP = true;
...
function testSoloSplit(win){
  ...
  const report = run(win, `
    ...
    /* 울림 보존 — 서스테인 원리라 지속음이 잘리지 않아야 한다 */
    let full = 0, sounding = 0;
    for(const tr of solo.tracks){
      const gs = tr.groups;
    ...
    return {
      ...
      counts,
      underLimit: counts.every(c => c <= 3000),
      ring: sounding / full
    };
  `);

  /* 솔로 = 파트를 합쳐 서스테인 원리로 성부를 나누고, 세 트랙 한도에 맞춘다.
     (예전의 음역별 3분할은 같은 음역 안 겹침을 잘라 지속음을 죽였다) */
  check("솔로는 트랙이 셋을 넘지 않는다", report.soloTracks >= 1 && report.soloTracks <= 3, "트랙 " + report.soloTracks);
  check("합주는 파트 그대로다", report.ensTracks === 2);
  check("트랙 이름이 '솔로'다", /^솔로/.test(report.labels), report.labels);
  check("음을 하나도 잃지 않는다", report.notesKept);
  check("전 트랙이 게임 한도 안이다", report.underLimit, report.counts.join("/"));
  check("지속음이 잘리지 않는다", report.ring > 0.95, "울림 " + (report.ring * 100).toFixed(0) + "%");
