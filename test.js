...
function testChordEditing(win){
  ...
  const report = run(win, `
    ...
    /* (4) 뒤쪽 손잡이를 끌면 화음이 함께 길어진다 */
    fresh([40, 44, 47]);
    pickAll();

    const el = document.querySelectorAll(".note")[0];
    ...
    fire("pointerdown", box.right - 2, el);

    const grabbed =
      pointerState &&
      pointerState.mode === "resize-end" &&
      pointerState.partners.length === 3;

    fire("pointermove", box.right - 2 + BAR_W * 0.25, el);
    fire("pointerup", box.right - 2 + BAR_W * 0.25, el);

    const grewTogether =
      tracks[0].notes.length === 3 &&
      tracks[0].notes.every(n => Math.abs(n.len - 0.5) < 1e-9);

    return {
      allHalf, cancelled, partialRefused, mergedRight,
      grabbed, grewTogether, tiedText
    };
  `);
  ...
  check("끝 손잡이를 잡으면 화음 전부가 딸려온다", report.grabbed);
  check("화음이 함께 길어진다", report.grewTogether);

}


/* ============================================================
   13. 이음줄 사슬은 하나로 다룬다 · 선택 모드에서 손잡이
   ============================================================
   c2&c4 는 화면에 막대 하나다. 복사·삭제·이동이 머리 토막만 다루면
   붙여넣은 음이 짧아지고, 지운 자리에 꼬리가 남아 "잘린" 것처럼
   보인다. 그리고 선택 모드에서 손잡이를 잡으면 길이 조절이어야지
   이동이 되면 안 된다. */
function testTieChainHandling(win){

  section("이음줄 사슬과 선택 모드 손잡이");
  ...
  const report = run(win, `
    ...
    tracks.length = 0;
    addTrack("피아노");
    [40, 44].forEach(r => tracks[0].notes.push({ bar:1, len:0.5, row:r, techs:[] }));
    normalizeAllNotes();
    refreshAll();
    setPickMode(true);
    multiSelection.clear();
    ...
    const resized =
      !!pointerState && pointerState.mode === "resize-end";

    fire("pointermove", box.right - 1 + BAR_W * 0.25, el);
    fire("pointerup", box.right - 1 + BAR_W * 0.25, el);

    const stayed =
      tracks[0].notes.every(n => n.bar === 1 && Math.abs(n.len - 0.75) < 1e-9);

    setPickMode(false);

    return { copiedAll, deletedAll, movingCount, resized, stayed };
  `);
  ...
  check("선택 모드에서 손잡이는 길이 조절이다", report.resized);
  check("그때 노트는 이동하지 않고 함께 길어진다", report.stayed);
