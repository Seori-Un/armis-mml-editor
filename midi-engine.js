...
/* 한 파트를 겹침 기준으로 성부로 나눈다 — 필요한 만큼 만든다.

   규칙은 하나다: "이미 있는 성부에 자리가 나면(앞 음이 끝났으면) 거기,
   아니면 새 성부". 자리가 없다고 앞 음을 자르는 일이 없으므로 모든
   음이 원본 길이 그대로 울린다 (서스테인 100% 보존).
   대가는 트랙 수 — 동시에 울리는 성부 수만큼 트랙이 생기고,
   게임에서는 한 사람이 트랙 셋이므로 여러 명이 나눠 연주하게 된다.

   화음 안에서도 "다음 음 전에 끝나는 음"(진행)과 "너머까지 울리는
   음"(지속)을 갈라 앉힌다 — 진행음 성부가 일찍 비므로 전체 성부
   수가 줄어든다. 폭주 방지로 파트당 성부는 12개까지만 만들고,
   그 너머는 가장 덜 잘리는 자리에 앉힌다 (사실상 안 일어난다). */
function splitPartByOverlap(part, ppq) {
  ...
  const place = (noteList, s, e) => {
    ...
    pick.notes.push(...noteList);
    if (e > pick.lastEnd) pick.lastEnd = e;
  };

  for (let i = 0; i < clusters.length; i++) {
    const c = clusters[i];
    const nextS = i + 1 < clusters.length ? clusters[i + 1].s : Infinity;

    const moving = [];
    const holding = [];
    for (const n of c.notes) {
      (toTick(n.endTick, ppq) <= nextS + 2 ? moving : holding).push(n);
    }

    if (moving.length) {
      place(
        moving, c.s,
        Math.max(...moving.map(n => toTick(n.endTick, ppq)))
      );
    }
    if (holding.length) {
      place(
        holding, c.s,
        Math.max(...holding.map(n => toTick(n.endTick, ppq)))
      );
    }
  }
  ...
  const base =
    part.soloLabel || ("T" + part.trackIndex + " · ch" + part.channel);
  const marks = ["①","②","③","④","⑤","⑥","⑦","⑧","⑨","⑩","⑪","⑫"];
  ...
  return voices
    ...
    .map((v, i) => ({
      ...
      soloLabel: base + " " + (marks[i] || (i + 1))
    }));
}

/* 겹침이 있는 파트를 전부, 필요한 만큼의 성부로 나눈다.
   트랙 수 제한 없음 — 게임에서는 3트랙 = 한 사람이므로 트랙이 많으면
   여러 명이 나눠 연주하면 된다. 겹침이 없는 파트는 그대로 두고,
   드럼(ch9)은 타격음이라 잘림이 소리에 안 남으므로 나누지 않는다. */
function allocateVoices(parts, ppq) {
  const out = [];
  for (const p of parts) {
    if (p.channel === 9 || partOverlapLoss(p, ppq) <= 0) {
      out.push(p);
    } else {
      out.push(...splitPartByOverlap(p, ppq));
    }
  }
  return out;
}
