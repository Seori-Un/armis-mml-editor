...
function makeGroups(part, instrument, ppq, techs, startOf) {
  ...
  for (const g of groups) {
    ...
    g.notes = trimChordNotes(g.notes);     // 게임 화음 상한 (CHORD_MAX 참고)
...
/* ============================================================
   화음 구성음 상한
   ============================================================
   게임 한 트랙이 동시에 낼 수 있는 음 수에는 상한이 있다.
   상한을 넘는 화음(:)을 적으면 게임에서 그 뒤의 음들이 밀린다 —
   실제 곡에서 편집기는 정확한데 인게임만 1분 30초부터 세 트랙이
   서로 어긋났고, 재 보니 6음 이상 화음이 처음 나오는 자리가
   정확히 그 시점(트랙별 87.9 / 90.6 / 94.3초)이었다. 5음까지의
   화음은 곡 첫머리부터 있었는데도 멀쩡했으므로 상한은 5로 본다.

   게임이 문서화한 값이 아니라 실측 추정이다. 나중에 다르다고
   확인되면 이 숫자 하나만 바꾸면 된다.

   버릴 때는 가장 낮은 음(베이스)을 남기고 높은 음부터 채운다 —
   가운데 겹침음이 빠지는 것이 귀에 가장 덜 띈다. */
let CHORD_MAX = 5;

/* key 오름차순으로 정렬된 구성음 배열을 상한에 맞춰 줄인다 */
function trimChordNotes(notes) {
  if (!(CHORD_MAX > 0) || notes.length <= CHORD_MAX) return notes;
  return [notes[0], ...notes.slice(-(CHORD_MAX - 1))];
}
