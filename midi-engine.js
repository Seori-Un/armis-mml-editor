...
/* 격자 굵기에 대해: 절반(1/64+1/48)으로 촘촘하게 하는 실험을 했었다.
   타이밍 흔들림은 평균 13ms→8ms로 줄지만 글자 수가 +8~10% 늘어,
   큰 곡에서 서스테인 트랙이 폭증하고(오케스트라 17→29개) 한도 초과가
   속출했다. 귀로 구분하기 어려운 이득에 비해 비용이 커서 이대로 둔다. */
/* 마비꼬(MabiIcco)와 같은 읽기: MidiFile.convTick — 모든 사건(음 시작·끝·
   템포)을 1/64음표(6틱) 격자로 반올림한다(parse_align=6, 반올림은 절반
   올림). 2분할·3분할을 따로 두던 예전 혼합 격자보다 촘촘해서 연주 녹음의
   흔들림이 반으로 줄고(최대 3틱), 셋잇단도 2틱 안에서 근사된다.
   화음 묶기(전역 시작점 지도)는 이 격자 위에서 그대로 작동한다 —
   아르미스는 화음을 c:e:g 로 적으므로 같이 친 음은 한 자리에 모아야 한다. */
const BINARY_GRID  = WHOLE / 64;   // 6 tick = 1/64음표 (마비꼬 parse_align=6)
const TRIPLET_GRID = WHOLE / 64;   // 같은 격자 — 셋잇단 전용 격자는 두지 않는다
...
/* 마비꼬는 자동 기법 판정(꾸밈음 흡수·글리산도·비브라토 등)을 하지 않고
   음을 있는 그대로 적는다. 같은 방식으로 간다 — 짧은 음이 기법으로
   바뀌어 사라지는 일이 없고, 편집기에서 사람이 원하는 자리에 기법을
   붙이면 된다. (true로 두면 예전처럼 자동 판정) */
let AUTO_TECHNIQUE = false;
...
/* ============================================================
   화음 상한 (다른 채팅에서 추가된 기능의 복원)
   ============================================================
   게임 한 트랙이 동시에 낼 수 있는 화음 구성음에는 상한이 있고,
   넘치면 그 뒤 음들이 밀린다. 그래서 출력 직전에 구성음을 상한
   개수로 줄인다 — 롤의 노트 데이터 자체는 건드리지 않는다.

   남기는 기준: 맨 아래(베이스)와 맨 위(멜로디)는 반드시 남기고,
   가운데는 세기가 큰 순서로 채운다. 화음의 뼈대(겉소리)를 지키는
   가장 안전한 선택이다.

   CHORD_MAX는 게임에서 실측한 값으로 맞춰야 한다. */
const CHORD_MAX = 5;

function trimChordNotes(notes) {
  if (!Array.isArray(notes) || notes.length <= CHORD_MAX) return notes;

  const byKey = [...notes].sort((a, b) => a.key - b.key);
  const keep = new Set([byKey[0], byKey[byKey.length - 1]]);

  const middle = byKey.slice(1, -1)
    .sort((a, b) => (b.velocity - a.velocity) || (b.key - a.key));

  for (const n of middle) {
    if (keep.size >= CHORD_MAX) break;
    keep.add(n);
  }

  return byKey.filter(n => keep.has(n));
}
...
function resolveTickTokensUncached(tick) {
  ...
  if (rem >= MIN_TICK) {
    /* 마비꼬 MMLTicks.makeMMLText 그대로: 표에 있으면 표, 없으면
       온음표부터 차례로 깎는다. (예전의 "가장 큰 값 집기" 대안은
       마비노기와 결과가 달라져서 쓰지 않는다) */
...
/* ============================================================
   변환 유틸
   ============================================================ */
/* 원본 tick → 내부 TPQN tick (MabiIcco MidiFile.convTick 그대로).
   마비꼬는 자바 정수 나눗셈이라 내림이다 — 반올림하면 미묘하게 다른
   자리로 떨어져 마비노기와 결과가 어긋난다. */
function toTick(tick, ppq) {
  if (!isFinite(ppq) || ppq <= 0) return 0;
  return Math.floor((tick * TPQN) / ppq);
}

/* 마비꼬 convTick의 정렬 그대로 (parse_align=6, 기본값).
     value += align/2;  value -= value % align;
   = 6틱(1/64음표) 단위로 반올림. 마비꼬가 하는 양자화는 이것뿐이다 —
   그 이상(격자 추측, 시작점 뭉치기 등)은 하지 않는다. */
const PARSE_ALIGN = 6;
function snap(tick) {
  let v = tick + (PARSE_ALIGN >> 1);
  v -= v % PARSE_ALIGN;
  return v;
}
...
function velocityToMml(v) {
  /* 마비꼬 MidiFile.parseShortMessage: velocity = data2 / 8 (0~15) */
  const [lo, hi] = velRange();
  return Math.max(lo, Math.min(hi, Math.trunc(v / 8)));
}
...
function collectParts(smf) {
  ...
  smf.tracks.forEach((events, trackIndex) => {
    ...
    /* 마비꼬 MidiFile.parseShortMessage 그대로:
       같은 음정이 이미 울리는 중이면 새 note-on은 무시하고(activeNoteMap
       containsKey 검사), note-off는 그 하나를 닫는다. */
    const closeNote = (ch, key, tick) => {
      const k = ch + ":" + key;
      const note = holding.get(k);
      if (!note) return;
    ...
    const openNote = (ch, key, vel, tick) => {
      const k = ch + ":" + key;
      if (holding.has(k)) return;   // 울리는 중 — 마비꼬처럼 무시
      holding.set(k, { key, velocity: vel, startTick: tick, endTick: tick, ch });
    };
...
function makeGroups(part, instrument, ppq, techs, startOf) {
  ...
  /* 시작점 뭉치기(전역 지도)는 아르미스 가공이지만 되살려 둔다 —
     6틱 정렬만으로는 살짝 어긋나게 친 화음이 트랙 사이에서 갈라져
     구르듯 들리는 문제가 있어서다. 지도 안의 격자 계산은 이제
     마비꼬 정렬(snap)을 그대로 쓴다. */
  const localStartOf =
  ...
  /* 마비꼬 최소 음길이: note-off까지가 6틱(1/64)이 안 되면 6틱으로.
     (MidiFile: tick < MMLTicks.minimumTick() → minimumTick) */
  for (const n of notes) {
    if (n.end < n.start + MIN_TICK) n.end = n.start + MIN_TICK;
  }
  ...
  if (techs.graceUp || techs.graceDown) {
    ...
    for (let i = 0; i < groups.length; i++) {
      ...
      if (next && g.notes.length === 1 && next.notes.length === 1) {
        ...
        /* 꾸밈음 자신도 짧아야 한다 — 본음 시작 언저리에서 손을 뗀
           음이어야지, 본음 너머까지 길게 울리는 음이면 그건 먼저 시작한
           멜로디/화음이다. 이 조건이 없으면 시작만 반 박 빠른 긴 음이
           통째로 흡수돼 사라진다. */
        const graceLen = g.notes[0].end - g.start;
...
/* ============================================================
   ...
   ============================================================ */
/* ============================================================
   틈 메우기 — 음을 다음 음까지 이어 울린다
   ============================================================
   연주를 녹음한 MIDI는 손을 뗀 순간에 음이 끝나, 그대로 적으면
   "짧은 음 + 쉼표"가 줄줄이 나와 뚝뚝 끊겨 들린다(쉼표가 전체의 40%를
   넘기도 한다). 사람이 치는 피아노는 페달과 잔향으로 그 틈이 이어진다.

   그래서 음 끝에서 다음 음 시작까지의 틈이 GAP_FILL_MAX 이하면 앞 음을
   다음 음까지 늘려 적는다. 그보다 긴 침묵은 진짜 쉼이므로 쉼표로 남긴다.
   서스테인 성부 나눔 뒤에 성부 안에서 적용되므로, 성부 나눔이 살린
   울림 위에 이 이어짐이 더해진다. 0으로 두면 예전처럼 원래 길이 그대로. */
let GAP_FILL_MAX = WHOLE / 2;   // 2분음표(192틱)까지의 틈은 메운다

/* 마비꼬는 음의 끝을 옮기지 않는다. 6틱 격자로 반올림된 끝을 그대로
   적는다 — 한 토막으로 안 떨어지면 이음줄(&)로 잇는다. */
let OPT_TIE_SNAP = false;
...
/* fillGaps: MIDI에서 불러올 때만 true — 편집기가 다시 만들 때는 사용자가
   그린 쉼표를 지켜야 하므로 false (app.js generateTrackMML 참고). */
function buildEvents(groups, tempos, instrument, techs, volumes, fillGaps) {
  ...
  groups.forEach((g, idx) => {
    ...
    /* 틈 메우기 (위 GAP_FILL_MAX 참고) — 다음 음까지 이어 울린다.
       구성음의 끝도 같이 늘린다 — 편집기 화면은 음의 끝(n.end)을 보고
       그리므로, 그룹 길이만 늘리면 화면은 짧고 MML만 길어 어긋난다. */
    if (fillGaps && next && GAP_FILL_MAX > 0) {
      const gap = slot - g.duration;
      if (gap > 0 && gap <= GAP_FILL_MAX) {
        g.duration = slot;
        for (const n of g.notes) {
          if (n.end < g.start + slot) n.end = g.start + slot;
        }
      }
    }
...
/* ── 편곡 (트랙 수·글자 수에 맞게 덜어내기) ──
   한 사람이 연주하려면 트랙 셋, 트랙마다 3000자다. 큰 곡은 음을 그대로
   두면 이 안에 못 들어간다. 그래서 사람이 편곡하듯 덜 중요한 음부터
   덜어낸다.

   무엇이 중요한가 — 한 순간에 같이 울리는 음들 중
     · 제일 높은 음: 멜로디. 없어지면 곡을 못 알아본다.        (가장 나중)
     · 제일 낮은 음: 베이스. 화성의 뿌리다.                     (그다음)
     · 가운데 음들: 화음을 채우는 소리. 빠져도 곡은 남는다.     (가장 먼저)
   같은 값이면 짧은 음부터 뺀다 — 스쳐 지나는 꾸밈음이 먼저 사라지고
   길게 끄는 음이 남아야 뼈대가 유지된다.

   덜어낼 음도 곧장 지우지 않는다. 다른 트랙에 글자 여유가 있으면 거기로
   옮긴다 — 옮긴 자리에서 앞뒤 음과 겹쳐 꼬리가 조금 잘리더라도 아예
   사라지는 것보다 낫고, 세 트랙의 9000자를 끝까지 쓰게 된다. */
function arrangeToBudget(song, maxTracks) {

  const ppq = song.ppq;

  /* 음마다 중요도를 매긴다 */
  const scoreNotes = part => {
    ...
    for (const c of clusters) {
      ...
      for (const n of c.notes) {
        const len = toTick(n.endTick, ppq) - toTick(n.startTick, ppq);
        let s;
        if (n.key === hi) s = 300;
        else if (n.key === lo) s = 200;
        else s = 100;
        score.set(n, s + Math.min(99, len / 4));
      }
    }
  ...
  for (let round = 0; round < 40; round++) {
    ...
    song.tracks.forEach((tr, i) => {
      ...
      if (
        gameCharCount(tr.mml) > GAME_CHAR_LIMIT &&
        (worst < 0 || gameCharCount(tr.mml) > gameCharCount(song.tracks[worst].mml))
      ) worst = i;
    });

    if (worst < 0) return true;   // 다 들어갔다

    const owner = song.parts[worst];
    const notes = owner.part.notes;

    if (notes.length < 8) return false;   // 더 뺄 게 없다

    const excess = gameCharCount(song.tracks[worst].mml) - GAME_CHAR_LIMIT;
    const perNote = gameCharCount(song.tracks[worst].mml) / notes.length;
    ...
    const drop = Math.max(
      ...
      Math.min(
        ...
        Math.ceil((excess / perNote) * 1.1)
      )
    );

    const score = scoreNotes(owner.part);

    const ranked = notes
      .slice()
      .sort((a, b) => (score.get(a) || 0) - (score.get(b) || 0));

    const picked = ranked.slice(0, drop);

    /* 지우기 전에 옮길 곳을 찾는다.
       글자에 여유가 가장 많은 다른 트랙(같은 악기)에 이 음들을 넘긴다.
       거기서 앞뒤 음과 겹쳐 조금 잘리더라도, 아예 사라지는 것보다 낫다.
       받는 쪽이 남은 여유만큼만 받는다 — 넘겨서 그쪽까지 넘치면 헛일이다. */
    let receiver = -1;
    let room = 0;
    ...
    song.tracks.forEach((tr, i) => {
      if (i === worst) return;
      if (song.parts[i].channel === 9) return;
      if (song.parts[i].part.program !== owner.part.program) return;
      const free = GAME_CHAR_LIMIT - 120 - gameCharCount(tr.mml);
      if (free > room) { room = free; receiver = i; }
    });

    let moved = [];

    if (receiver >= 0 && room > perNote * 4) {

      const canTake = Math.floor(room / perNote);
      moved = picked.slice(0, Math.min(picked.length, canTake));

      const target = song.parts[receiver];
      target.part.notes = target.part.notes
        .concat(moved)
        .sort((a, b) => a.startTick - b.startTick);
      target.noteCount = target.part.notes.length;

    }

    const gone = new Set(picked);

    owner.part.notes = notes.filter(n => !gone.has(n));
    owner.noteCount = owner.part.notes.length;
...
/* ── 성부를 정해진 트랙 수로 줄이기 ──
   서스테인 모드는 잘림 없이 담느라 성부를 필요한 만큼 만든다. 한 사람이
   연주하려면 트랙이 셋뿐이므로, 그보다 많으면 합쳐야 한다.

   합칠 때는 "가장 덜 다치는 짝"부터 고른다. 두 성부를 한 트랙에 앉히면
   서로의 지속음이 상대의 다음 음에서 잘리는데, 그 잘리는 총량(tick)이
   적은 짝일수록 원곡에 가깝게 남는다. 겹침이 아예 없는 짝은 손실 0이라
   가장 먼저 합쳐진다. */
function mergeVoicesToLimit(parts, ppq, limit) {

  const loss = (a, b) => {
    const merged = chordClustersOf(a.notes.concat(b.notes), ppq);
    let sum = 0;
    for (let i = 0; i + 1 < merged.length; i++) {
      const nextS = merged[i + 1].s;
      for (const n of merged[i].notes) {
        sum += Math.max(0, toTick(n.endTick, ppq) - nextS);
      }
    }
    return sum;
  };

  const pool = parts.map(p => ({ ...p, notes: p.notes.slice() }));
  ...
  while (pool.length > limit) {
    ...
    for (let i = 0; i < pool.length; i++) {
      for (let j = i + 1; j < pool.length; j++) {
        const v = loss(pool[i], pool[j]);
        if (v < best) { best = v; bi = i; bj = j; }
      }
    }

    pool[bi].notes = pool[bi].notes
      .concat(pool[bj].notes)
      .sort((a, b) => a.startTick - b.startTick);

    pool[bi].mods = (pool[bi].mods || [])
      .concat(pool[bj].mods || [])
  ...
  const marks = ["①", "②", "③", "④", "⑤", "⑥"];
  const base =
    (pool[0].soloLabel || "").replace(/ [①-⑫]$/u, "") ||
    ("T" + pool[0].trackIndex + " · ch" + pool[0].channel);

  pool.forEach((p, i) => {
    p.soloLabel = base + (pool.length > 1 ? " " + (marks[i] || (i + 1)) : "");
  });
...
/* ── 글자 수 균형 (서스테인 모드) ──
   ...
   들 때까지 반복한다(최대 60바퀴). 단, 초과 트랙 수가 세 바퀴째
   줄지 않으면 승산이 없는 것이므로 멈춘다 — 템포 변경이 매우 잦은
   곡은 템포 명령만으로 트랙당 한도를 넘어서(모든 트랙에 템포가
   들어가는 게임 규칙), 아무리 쪼개도 한도 안에 들 수 없다.
   드럼은 건드리지 않는다. */
function balanceVoiceChars(song, allowGrow, maxTracks) {

  const marks = ["①","②","③","④","⑤","⑥","⑦","⑧","⑨","⑩","⑪","⑫"];

  let prevExcess = Infinity;
  let stuck = 0;

  /* 템포 명령은 게임 규칙상 모든 트랙에 복사된다. 템포 변경이 아주
     잦은 곡(루바토 녹음)은 템포만으로 트랙당 한도를 넘어서, 새 트랙을
     만들어 봐야 그 트랙도 태어나자마자 초과다. 그런 곡에서는 새 트랙
     만들기(②)를 봉인하고 옮기기(①)만 시도한다. */
  const canGrow =
    allowGrow !== false &&
    (song.tempoList ? song.tempoList.length : 0) * 4 <= GAME_CHAR_LIMIT;
  ...
  for (let round = 0; round < 60; round++) {
    ...
    // 가장 초과가 큰 트랙 + 진전 감시 (잣대 = 초과 글자 총량)
    let worst = -1;
    let excessTotal = 0;
    song.tracks.forEach((tr, i) => {
      if (song.parts[i].channel === 9) return;
      if (gameCharCount(tr.mml) > GAME_CHAR_LIMIT) {
        excessTotal += gameCharCount(tr.mml) - GAME_CHAR_LIMIT;
        if (worst < 0 || gameCharCount(tr.mml) > gameCharCount(song.tracks[worst].mml)) worst = i;
      }
    });
    if (worst < 0) return;   // 전부 한도 안

    /* 지난 최저치를 못 깨면 정체로 본다 — 둘 사이를 오가는 탁구도 잡힌다 */
    if (excessTotal >= prevExcess) {
      stuck++;
      if (stuck >= 3) return;   // 세 바퀴째 못 줄였다 — 승산 없음
    } else {
      stuck = 0;
      prevExcess = excessTotal;
    }

    const over = song.parts[worst];
    const excess = gameCharCount(song.tracks[worst].mml) - GAME_CHAR_LIMIT;
    const perNote =
      gameCharCount(song.tracks[worst].mml) / Math.max(1, over.part.notes.length);
    ...
    /* ① 같은 악기의 여유 트랙으로 — 글자 적은 곳부터.
       받는 쪽은 "남은 여유만큼만" 받는다(room). 여유를 안 재고 넘기면
       받는 쪽이 넘쳐 다음 바퀴에 도로 돌려보내는 탁구가 벌어진다. */
    const receivers = song.parts
      .map((p, i) => ({
        p, i,
        room: Math.floor(
          (GAME_CHAR_LIMIT - 150 - gameCharCount(song.tracks[i].mml)) / perNote
        )
      }))
      .filter(x =>
        x.i !== worst &&
      ...
      .sort((a, b) => gameCharCount(song.tracks[a.i].mml) - gameCharCount(song.tracks[b.i].mml));
    ...
    if (moved.size) {
      ...
      if (!canGrow) return;   // 템포만으로 한도 초과인 곡 — 만들어야 헛일
      if (maxTracks > 0 && song.parts.length >= maxTracks) return;   // 트랙 한도
...
function allocateVoices(parts, ppq) {
  ...
  for (const p of parts) {
    ...
    /* program 지정이 없는 파트는 GM 관례대로 0(피아노)으로 본다.
       String(null)="null"로 묶으면, 지정만 안 했을 뿐 서로 다른
       악기인 파트들이 "같은 악기"로 합쳐지는 사고가 난다 — 0으로
       보면 명시적 피아노와 자연스럽게 한 살림이 되고, 악기를 바꾼
       파트와는 안 섞인다. */
    const key = String(p.program === null ? 0 : p.program);
...
function convert(buffer, options) {
  ...
  /* 솔로곡: 한 사람이 트랙 셋으로 다 연주한다.
     예전에는 음역(높은음/가운데/낮은음)으로 셋으로 갈랐는데, 그러면 같은
     음역 안의 겹침이 그대로 잘려 지속음이 죽는다. 이제는 서스테인 모드와
     같은 원리로 간다 — 파트를 전부 합친 뒤 겹치는 음을 성부로 갈라
     지속음을 살리고, 성부가 셋을 넘으면 가장 덜 다치는 짝부터 합치고,
     그래도 글자가 넘치면 편곡기가 덜 중요한 음부터 덜어낸다(아래 끝). */
  ...
  const limit =
    solo ? MAX_TRACKS
    : (options && options.mode === "voices") ? (options.fitTracks | 0)
    : 0;

  if (limit > 0) {

    /* 트랙 수에 한도가 있다(솔로·AI 채보 = 한 사람 연주용 셋).
       한도 안에서는 트랙을 늘려 가며 옮기고, 한도에 닿으면 편곡기가
       덜 중요한 음부터 덜어내 글자 수를 맞춘다. */
    balanceVoiceChars(song, true, limit);
    arrangeToBudget(song, limit);

  } else if (options && options.mode === "voices") {
...
function rebuild(song) {
  ...
  for (const { p, groups, techs } of prepared) {
    /* MIDI 불러오기 경로 — 틈 메우기를 켠다 (buildEvents 위 설명) */
    const events = buildEvents(groups, tempoList, p.armis, techs, undefined, true);
