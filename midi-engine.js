...
function makeGroups(part, instrument, ppq, techs) {
  ...
  /* ── 화음 묶기 ──
     "1/64음표보다 가까운 시작점은 한 화음"이 의도인데, 예전에는
     격자에 맞춘 뒤(start)의 간격으로 판정했다. 그러면 사람이 동시에
     누른 화음(원본 십몇 ms 차이)이 하이브리드 스냅에서 2분할·3분할
     격자로 각각 붙어 8틱씩 벌어지고, 벌어진 뒤라 화음으로 안 묶여
     구르듯 갈라져 들렸다("음이 밀린다"). 그래서:

       1. 격자에 맞추기 전 위치(raw)로 먼저 묶고
       2. 묶음의 가운데 음 자리를 한 번만 격자에 맞춰
          구성음 전체가 같은 시작점을 갖게 한다.

     묶는 창은 묶음의 첫 음 기준이다 — 음마다 창을 이어 붙이면
     빠른 연속음이 사슬처럼 전부 한 덩어리가 되기 때문이다. */
  let groups = [];
  for (const n of notes) {
    const last = groups[groups.length - 1];
    if (last && n.raw - last.rawFirst < MIN_TICK) {
      ...
      groups.push({ start: 0, rawFirst: n.raw, notes: [n], techniques: [] });
    }
  }

  /* 묶음의 시작점: 구성음 raw의 가운데 값을 격자에 맞춘다.
     구성음의 start도 여기에 맞춘다 (아래 단계들이 그룹 시작을 쓰므로). */
  for (const g of groups) {
    const raws = g.notes.map(x => x.raw).sort((a, b) => a - b);
    g.start = snap(raws[Math.floor((raws.length - 1) / 2)]);
    delete g.rawFirst;
    for (const x of g.notes) x.start = g.start;
    g.notes.sort((a, b) => a.key - b.key);
  }

  /* 스냅 때문에 이웃 묶음이 같은 시작점으로 겹칠 수 있다 — 합친다 */
  groups.sort((a, b) => a.start - b.start);
  {
    const merged = [];
    for (const g of groups) {
      const last = merged[merged.length - 1];
      if (last && last.start === g.start) {
        ...
        last.notes.sort((a, b) => a.key - b.key);
      } else {
        merged.push(g);
      }
    }
