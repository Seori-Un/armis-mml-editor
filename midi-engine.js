...
function arrangeToBudget(song, maxTracks) {
  ...
  for (let round = 0; round < 40; round++) {
    ...
    const perNote = song.tracks[worst].mml.length / notes.length;
    ...
    song.tracks.forEach((tr, i) => {
      ...
      const free = GAME_CHAR_LIMIT - 120 - tr.mml.length;
...
function balanceVoiceChars(song, allowGrow, maxTracks) {
  ...
  for (let round = 0; round < 60; round++) {
    ...
    song.tracks.forEach((tr, i) => {
      if (song.parts[i].channel === 9) return;
      if (tr.mml.length > GAME_CHAR_LIMIT) {
        excessTotal += tr.mml.length - GAME_CHAR_LIMIT;
        if (worst < 0 || tr.mml.length > song.tracks[worst].mml.length) worst = i;
      }
    });
    ...
    const receivers = song.parts
      .map((p, i) => ({
        p, i,
        room: Math.floor(
          (GAME_CHAR_LIMIT - 150 - song.tracks[i].mml.length) / perNote
        )
      }))
      .filter(x =>
        x.i !== worst &&
