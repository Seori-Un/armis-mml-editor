...
try{
  importScripts(${JSON.stringify(TR_ORT_CDN + "ort.min.js")});
}catch(e){
  /* 실행기(onnxruntime-web)를 못 불러왔다 — 망이 막혔거나 CDN 차단 */
  postMessage({
    type: "error",
    code: "no-runtime",
...
async function loadModelOnce(useCache){
  ...
  if(cached){
    ...
    let at = 0;
    for(const p of parts){ weights.set(p, at); at += p.byteLength; }
  ...
  try{
    return await ort.InferenceSession.create(graph.buffer.slice(0), {
    ...
    throw {
      ...
      message:
        typeof e === "number"
          ? "모델을 여는 중 실패했습니다 (코드 " + e + "). model.onnx가 최신인지 확인해 주세요."
...
function startAudioTranscription(file, onProgress){
  ...
  const promise = (async () => {
    ...
    const blob =
      new Blob([trWorkerSource()], { type: "text/javascript" });
