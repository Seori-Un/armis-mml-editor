...
/* ── 서버 채보 (Transkun V2) ──
   오디오 파일을 그대로 서버(/transcribe)로 보내면, Cloudflare
   컨테이너 안의 파이썬 원본 transkun이 채보해 MIDI 바이트를
   돌려준다. 브라우저 모델(ByteDance)보다 정확하고, 기기가
   계산을 전혀 안 한다.

   실패하면(주소 없음, 옛 서버라 404, 컨테이너 미배포 501,
   오프라인, 파일 너무 큼 등) null을 돌려주고, 부른 쪽이
   기존 브라우저 채보로 조용히 물러선다.

   주의: 이 길을 타면 오디오가 서버로 올라간다. 브라우저 길은
   기기 밖으로 아무것도 안 보낸다 — 그 성질은 폴백으로 남는다. */
const TR_SERVER_MAX_BYTES = 50 * 1024 * 1024;   // worker.js와 같은 값
const TR_SERVER_TIMEOUT = 20 * 60 * 1000;       // 채보는 곡당 수 분 걸린다

/* 서버 채보의 "예상 진행률"에 쓰는 값들.
   서버(transkun)는 중간 보고를 안 해주므로, 곡 길이 × 배속으로
   전체 시간을 어림해 막대를 채운다. 채보가 끝날 때마다 실제
   배속을 기억해(localStorage) 다음 곡부터 예상이 점점 정확해진다. */
const TR_SERVER_RATIO_KEY = "trServerRatio";    // 곡 1초당 처리 시간(초)
const TR_SERVER_RATIO_DEFAULT = 3.0;
const TR_SERVER_WARMUP = 25;                    // 시동·전송 몫(초)

function trServerRatio(){

  try{
    const v = parseFloat(localStorage.getItem(TR_SERVER_RATIO_KEY));
    if(isFinite(v) && v > 0.05 && v < 20){
      return v;
    }
  }catch(_){ }

  return TR_SERVER_RATIO_DEFAULT;

}

function trSaveServerRatio(measured){

  try{

    /* 급변을 피하려고 이전 값과 절반씩 섞는다 */
    const next =
      (trServerRatio() + measured) / 2;

    localStorage.setItem(
      TR_SERVER_RATIO_KEY,
      String(Math.max(0.05, Math.min(20, next)))
    );
...
/* 오디오 길이(초)만 가볍게 알아낸다 — 전체 해독 없이 메타데이터만.
   실패하면 0을 돌려주고, 그 경우 막대는 흐르는 모양으로 물러선다. */
function trAudioDuration(file){
  ...
  return new Promise(resolve => {
    ...
    try{
      ...
      /* 메타데이터를 영영 못 읽는 형식도 있다 — 5초만 기다린다 */
...
async function trTranscribeOnServer(file, onProgress, holder){

  /* 서버 주소는 MIDI 변환과 같은 곳(app.js의 MIDI_API_URL)을 쓴다 */
  if(typeof MIDI_API_URL === "undefined" || !MIDI_API_URL){
    return null;
  }

  if((file.size || 0) > TR_SERVER_MAX_BYTES){
    return null;
  }

  const base =
    MIDI_API_URL.endsWith("/")
      ? MIDI_API_URL
      : MIDI_API_URL + "/";

  /* 일감 아이디 — 서버가 "어느 채보를 죽일지" 알아보는 표식 */
  const jobId =
    (crypto.randomUUID
      ? crypto.randomUUID()
  ...
  const cancelUrl =
    base + "transcribe/cancel?job=" + encodeURIComponent(jobId);

  /* 서버에 취소를 알린다. 응답은 기다리지 않는다(발사 후 망각).
     keepalive/sendBeacon 덕에 페이지가 닫히는 중에도 전송된다. */
  const tellServerCancel = () => {
  ...
  /* 채보 중에 페이지를 닫거나 떠나면 서버 작업도 같이 멈춘다 */
  const onPageHide = () => tellServerCancel();
  ...
  /* ── 예상 진행률 ──
     곡 길이 × 기억해 둔 배속 + 시동 몫 = 전체 예상 시간.
     0.5초마다 경과/예상을 알린다. 실제보다 빨리 차 버리지 않게
     97%에서 멈춰 세우고, 응답이 오면 그대로 다음 단계로 넘어간다. */
  const durationSec =
    await trAudioDuration(file);

  const estimate =
    durationSec > 0
      ? TR_SERVER_WARMUP + durationSec * trServerRatio()
      : 0;
  ...
  if(estimate > 0){
    ...
    /* 예상이 빗나가 실제가 더 오래 걸리면, 예상치를 그때그때
       늘려 잡는다(경과의 1.18배). 막대는 97%에서 굳는 대신
       ~85% 언저리에서 계속 조금씩 움직이고, 남은 시간도 매번
       다시 계산된다 — "멈췄나?" 싶은 순간을 없애기 위한 것이다 */
    let liveEstimate = estimate;
    ...
    const tick = () => {
      ...
      if(elapsed > liveEstimate * 0.9){
        liveEstimate = elapsed * 1.18;
      }

      const shown =
        Math.min(elapsed, liveEstimate * 0.97);

      onProgress({
        stage: "server",
        done: shown,
        total: liveEstimate,
        extra: Math.max(1, Math.round(liveEstimate - elapsed))
      });
    ...
    onProgress({ stage: "server" });

  }

  try{

    /* File 객체를 그대로 body에 주면 일부 브라우저(특히 Safari)가
       빈 몸통으로 보내는 일이 있다. 바이트로 확실히 읽어서 보낸다. */
    const payload =
      await file.arrayBuffer();

    if(!payload || payload.byteLength === 0){
      console.warn("파일을 읽지 못해 브라우저 채보로 전환");
      return null;
    }

    const resp =
      await fetch(
        base + "transcribe?job=" + encodeURIComponent(jobId),
        {
          method: "POST",
          body: payload,
          signal: ctrl.signal
    ...
    if(!resp.ok){
      ...
      /* 서버가 거절한 이유를 콘솔에만 남긴다 — 화면은 곧바로
         브라우저 채보로 이어지므로 사용자를 붙잡지 않는다 */
      console.warn(
        "서버 채보 불가(" + resp.status + "), 브라우저 채보로 전환:",
        await resp.text().catch(() => "")
      );

      /* 사용자에게 이유를 한 줄 알린다 — 429(모두 사용 중)면
         "붐빔", 그 외는 "문제" */
      onProgress({
        stage: "fallback",
        reason: resp.status === 429 ? "busy" : "error"
      });
    ...
    /* 표준 MIDI 머리("MThd")인지 한 번 확인한다. 프록시가 끼어
       엉뚱한 HTML을 돌려주는 망에서 이상한 파일을 여는 일을 막는다 */
    if(
      bytes.length < 4 ||
      bytes[0] !== 0x4d || bytes[1] !== 0x54 ||
      bytes[2] !== 0x68 || bytes[3] !== 0x64
    ){
      console.warn("서버 응답이 MIDI가 아님, 브라우저 채보로 전환");
      return null;
    }

    /* 실제 배속을 기억해 다음 곡의 예상을 다듬는다.
       비정상적으로 빨리 온 응답(서버 결과 캐시 히트)은 진짜
       채보 속도가 아니므로 건너뛴다 — 안 그러면 다음 곡의
       예상이 터무니없이 낙관적이 된다 */
    if(durationSec > 0){
      ...
      if(took > TR_SERVER_WARMUP + 5){

        trSaveServerRatio(
          Math.max(0.05, (took - TR_SERVER_WARMUP) / durationSec)
        );
    ...
    /* 사용자가 취소한 경우는 그대로 위로 던진다 — 폴백하지 않는다 */
    if(holder.cancelled){
      throw err;
    }

    console.warn("서버 채보 실패, 브라우저 채보로 전환:", err);

    onProgress({ stage: "fallback", reason: "error" });
...
/* ── 공개 함수: 파일 → MIDI 바이트 ──
   ...
   stage: server | decode | model | session | transcribe | post | build */
function startAudioTranscription(file, onProgress){

  let worker = null;

  /* 서버 요청 취소 손잡이. promise.cancel이 여기를 통해
     진행 중인 fetch를 끊는다. */
  const serverHolder = { abort: null, cancelled: false };

  const promise = (async () => {

    /* ① 서버(컨테이너의 파이썬 transkun)를 먼저 시도한다.
       성공하면 기기는 계산을 전혀 안 하고 MIDI를 바로 받는다.
       실패하면 null — 아래 브라우저 채보가 그대로 이어진다. */
    const serverMidi =
      await trTranscribeOnServer(file, onProgress, serverHolder);

    if(serverMidi){
      return serverMidi;
    }

    /* ② 브라우저 채보 (기존 ByteDance 모델, 기기 안에서만) */
  ...
  promise.cancel = () => {
    serverHolder.cancelled = true;
    if(serverHolder.abort){
      serverHolder.abort();
    }
    if(worker){
      worker.terminate();
