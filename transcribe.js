...
const TR_MODEL_CACHE_KEY = "bytedance-piano-v2";
const TR_ORT_CDN =
...
async function trDecodeAudio(file){
...
async function loadModelOnce(useCache){
  ...
  let graph, weights;
  let saveAfterOpen = false;
  if(cached){
    graph = cached.graph;
    weights = cached.weights;
  }else{
    /* 가중치는 20MB 조각 여러 개다 (깃허브 웹 업로드 한도 25MB에 맞춘 크기).
       하나씩 줄 세워 받으면 느리므로, 앞의 몇 개를 한꺼번에 받아 보고
       404가 난 지점에서 멈춘다. 조각 개수를 코드에 박지 않는 이유는
       모델을 바꾸면 개수가 달라지기 때문이다. */
    let got = 0;
    const onBytes = n => { got += n; report("model", got, 0); };

    /* 그래프 파일에는 표식을 붙여 받는다 — 브라우저가 저장해 둔 옛
       model.onnx를 그대로 내주는 것을 막는다. 가중치(weights.bin.N)는
       내용이 그대로이므로 표식 없이 저장본을 재활용한다. */
    graph = await fetchWithProgress(
      MODEL_DIR + "model.onnx?v=" + CACHE_VERSION, onBytes
    );

    const parts = [];
    const LANES = 4;          // 동시에 받을 조각 수
    let done = false;
    ...
    for(let base = 0; !done; base += LANES){
      ...
      for(let i = 0; i < LANES; i++){
        jobs.push(
          fetchWithProgress(MODEL_DIR + "weights.bin." + (base + i), onBytes)
      ...
      const got2 = await Promise.all(jobs);
    ...
    if(!parts.length){
      throw { code: "no-model", url: MODEL_DIR + "weights.bin.0" };
    }

    /* 메모리를 아낀다 — 폰에서는 이 대목이 고비다.
       조각(154MB)과 합본(154MB)을 동시에 들고 있으면, 여기에 세션이
       WASM 안에 또 한 벌을 만들면서 한계를 넘어 죽는다(숫자 예외).
       그래서 한 조각씩 옮겨 담고 즉시 놓아준다. 기기에 저장하는 일도
       세션을 무사히 연 뒤로 미룬다 — 저장은 또 한 벌을 복사한다. */
    let total = 0;
    for(const p of parts) total += p.byteLength;

    weights = new Uint8Array(total);

    let at = 0;
    for(let i = 0; i < parts.length; i++){
      weights.set(parts[i], at);
      at += parts[i].byteLength;
      parts[i] = null;      // 옮긴 조각은 바로 놓아준다
    }
    parts.length = 0;

    saveAfterOpen = true;   // 세션을 연 뒤에 저장한다
  }
  report("session", 0, 1);
  /* 그래프 최적화는 "꺼 둔다".
     이 모델은 브라우저 런타임이 최적화를 시도하면 WASM 내부에서 터진다
     (숫자만 던지는 예외). 그래서 model.onnx를 만들 때 미리 최적화를
     끝내 두고(convert_model.py), 여기서는 그대로 싣기만 한다.
     최적화는 연산을 합칠 뿐이라 결과는 한 치도 다르지 않다 — 실제로
     최적화 전후 출력이 완전히 같은 것을 확인했다. */
  try{
    const session = await ort.InferenceSession.create(graph.buffer.slice(0), {
      executionProviders: ["wasm"],
      graphOptimizationLevel: "disabled",
      externalData: [{ path: "weights.bin", data: weights }]
    });

    /* 무사히 열렸으니 이제 저장해 둔다 (다음부터 안 내려받게).
       저장이 실패해도(저장 공간 부족 등) 그냥 넘어간다 — 다음에 다시
       내려받으면 그만이고, 여기서 멈출 이유가 없다. */
    if(saveAfterOpen){
      await idbPut(CACHE_KEY, { graph, weights });
    }

    return session;
  }catch(e){
    /* WASM 내부 예외는 숫자로만 온다 — 사람이 읽을 말로 바꿔 준다 */
    throw {
      code: "session",
      detail:
        typeof e === "number" ? String(e) : "",
      message:
        typeof e === "number"
          ? "session " + e
...
function startAudioTranscription(file, onProgress){
  ...
  const promise = (async () => {
    ...
    const wave = await trDecodeAudio(file);
    ...
    const { notes, pedals } = await new Promise((resolve, reject) => {
      ...
      /* 파형을 통째로 넘긴다 (소유권 이전으로 복사 없이).
         모델 주소는 여기서 절대 주소로 만들어 함께 보낸다 — 워커가
         blob:이라 상대 주소를 스스로 풀 수 없기 때문이다. */
      worker.postMessage(
        {
          type: "start",
          wave,
