...
function collabOnLocalChange(){
  ...
  /* 대곡 MIDI를 방에서 불러오면 곡 전체가 op로 나간다. 서버는 4MB를
     넘는 메시지를 버리므로(조용히 동기화가 깨진다), 1MB쯤에서 잘라
     여러 번 보낸다. 같은 연결에서는 순서가 지켜져 안전하다. */
  ...
  const flush = ()=>{
    ...
    try{
      ...
      collabWs.send(
        JSON.stringify({ type:"ops", ops: batch })
      );
  ...
  for(const op of ops){
    ...
    if(batch.length && size + opSize > CHUNK_BYTES){
      flush();
    }
...
function collabLeave(showMessage){
  ...
  /* 드래그 중 미뤄 둔 남의 편집이 남아 있으면 버린다 —
     안 그러면 방을 나간 뒤에 뒤늦게 내 곡에 적용된다 */
  collabQueue = [];
  if(collabQueueTimer){
    clearInterval(collabQueueTimer);
    collabQueueTimer = null;
