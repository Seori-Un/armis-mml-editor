...
/* ============================================================
   ...
   보내는 쪽 — 0.35초마다, 바뀌었을 때만:
     sel    내가 보고 있는 트랙 cid
     cursor 내 재생선(커서) 마디 위치
     notes  내가 잡고 있는 노트들의 [cid, bar, row, len]
            (드래그 중에도 노트 실물의 bar/row가 계속 바뀌므로,
             이걸 계속 보내면 상대 화면에서 움직이는 게 그대로 보인다)

   받는 쪽 — 사람마다:
     · 색 세로선 + 이름표 = 그 사람의 커서
     · 색 테두리 상자 = 그 사람이 잡은 노트들
     · 트랙 목록의 색 점 = 누가 어느 트랙에 있는지
   ============================================================ */

const COLLAB_PRESENCE_MS = 350;
const COLLAB_PRESENCE_STALE_MS = 8000;   // 이만큼 소식 없으면 표시를 걷는다
const COLLAB_PRESENCE_MAX_NOTES = 60;

/* id → { name, color, sel, cursor, notes, at } */
...
setInterval(
  () => {
    ...
    // 남의 표시도 이 박자에 맞춰 되그린다 (확대/축소 변화 반영 겸)
    collabRenderPresence();
...
/* 피아노롤 안에 프레즌스 층을 (없으면) 만든다.
   grid와 같은 좌표계라 노트와 같은 계산식으로 자리를 잡는다. */
...
function collabRenderPresence(){
  ...
  for(const [id, p] of collabPresence){
    ...
    /* 그 사람 트랙이 내 화면에서 꺼져 있으면 노트·커서는 안 그린다
       (안 보이는 트랙 위 좌표라 헛자리에 뜬다) */
    const trackVisible =
      trackIndex >= 0 &&
      tracks[trackIndex] &&
    ...
    if(typeof p.cursor === "number"){
      ...
      line.className = "collab-peer-cursor";
      line.style.left = (p.cursor * BAR_W) + "px";
      line.style.background = p.color || "#888";

      const flag =
        document.createElement("span");

      flag.className = "collab-peer-flag";
      flag.textContent = p.name || "?";
      flag.style.background = p.color || "#888";
